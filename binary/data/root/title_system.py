...
face_images = {
	...
	playerSettingModule.RACE_WOLFMAN_M	: "d:/ymir work/TitleSystem/faces/lycan.tga",
}
...
class Title_System(ui.ScriptWindow):
	...
	def Load(self):
		...
		self.Board.SetTitleName("Ünvan Bilgi Tablosu")
...
##############################################################################################	
		...
		self.border_prestige = ui.ImageBox()
		self.border_prestige.SetParent(self.Board)
		self.border_prestige.SetPosition(120,180)
		self.border_prestige.Show()
		self.border_prestige.LoadImage("d:/ymir work/TitleSystem/chenare/chenar_bun.png")	
		...
		self.border_prestige_2 = ui.ImageBox()
		self.border_prestige_2.SetParent(self.Board)
		self.border_prestige_2.SetPosition(120,240)
		self.border_prestige_2.Show()
		self.border_prestige_2.LoadImage("d:/ymir work/TitleSystem/chenare/chenar_bun.png")			
		
		self.current_title = ui.TextLine()
		self.current_title.SetParent(self.border_prestige)
		self.current_title.SetPosition(-100,0)
		self.current_title.SetText(localeInfo.TITLE_CURRENT_NAME)
		self.current_title.SetPackedFontColor(0xffe0f283)
		self.current_title.Show()	
		
		self.current_money = ui.TextLine()
		self.current_money.SetParent(self.chenar_total_yang)
		self.current_money.SetPosition(65,5)
		self.current_money.SetText(localeInfo.NumberToMoneyString(money))
		self.current_money.SetHorizontalAlignCenter()
		self.current_money.Show()
		
		self.current_playtime = ui.TextLine()
		self.current_playtime.SetParent(self.border_prestige_2)
		self.current_playtime.SetPosition(65,5)
		self.current_playtime.SetText(str(player.GetPlayTime()) + "222")
		self.current_playtime.SetHorizontalAlignCenter()
		self.current_playtime.Show()		
		
		self.current_amount = ui.TextLine()
		self.current_amount.SetParent(self.current_money)
		self.current_amount.SetPosition(-165,0)
		self.current_amount.SetText(localeInfo.TITLE_TOTAL_AMOUNT)
		self.current_amount.SetPackedFontColor(0xffe0f283)
		self.current_amount.Show()
		
		self.text_minutes = ui.TextLine()
		self.text_minutes.SetParent(self.current_playtime)
		...
		self.text_level.SetText("Seviyeniz:")
...
############################################################################3
	...
	def __AppendPrestige(self):	
		...
		self.blue_legion.SetParent(self.border_prestige)
...
##############################################################################################
	...
	def NonSelectBox(self):		
		self.wndDialog = uiCommon.PopupDialog()
		self.wndDialog.SetWidth(550)
		self.wndDialog.SetText(localeInfo.TITLE_WARNING_DIALOG)
		self.wndDialog.Show()
...
##############################################################################################		
	...
	def BuyPrestige(self):
		...
		if list_name_prestige:
			...
			self.wndDialog = uiCommon.PopupDialog()
			self.wndDialog.SetWidth(550)
			self.wndDialog.SetText(localeInfo.TITLE_QUESTWARNING_POTION)
			self.wndDialog.Show()
...
##############################################################################################	
	...
	def OnUpdate(self):
		if self.Board.IsShow():
			...
			self.current_money.SetText(localeInfo.NumberToMoneyString(prestige_money))
			self.current_playtime.SetText(str(player.GetPlayTime()) + "")
			self.level.SetText(str(player.GetStatus(player.LEVEL)))
