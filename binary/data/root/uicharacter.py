...
if localeInfo.IsYMIR():
	SHOW_LIMIT_SUPPORT_SKILL_LIST = [122, 143, 144, 145, 146, 147, 148, 131, 137, 138, 139, 140, 141, 142, 149, 121]
...
class CharacterWindow(ui.ScriptWindow):
	...
	def __ClickEmotionSlot(self, slotIndex):
		...
		if slotIndex > 50 and slotIndex <= 53:
	...
	def RefreshStatus(self):
		...
		try:
			if player.GetRebirt() > 0:
				text_amk = str(player.GetStatus(player.LEVEL)) + str("-") + str(player.GetRebirt())
				self.GetChild("Level_Value").SetText(str(text_amk))
			else:
	...
	def RefreshAlignment(self):
		...
		self.toolTipAlignment.AutoAppendTextLine("Büyülü Saldýrý/Yakýn Dövüþ Saldýrý: " + str(p2),POSITIVE_COLOR)
		self.toolTipAlignment.AutoAppendTextLine("Metinlere Karþý Güçlü : " + str(p2),POSITIVE_COLOR)
		self.toolTipAlignment.AutoAppendTextLine("Bosslara Karþý Güçlü : " + str(p2),POSITIVE_COLOR)
