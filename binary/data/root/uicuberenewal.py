...
def FReturnInfo(func,index):
	(vnum_reward,count_reward,item_reward_stackable,vnum_material_1,count_material_1,vnum_material_2,count_material_2,vnum_material_3,count_material_3,vnum_material_4,count_material_4,vnum_material_5,count_material_5,cheque,gem,gem2,percent,category) = cube_renewal.GetDates(index)
	info = {
	...
	def CountTotal(self):
		...
		for i in xrange(1,6):
			if self.GetVnumMaterial(i) != 0:
				adet = player.GetItemCountByVnum(self.GetVnumMaterial(i))
				adet2 = player.GetItemCountByUpgradeVnum(self.GetVnumMaterial(i))
				adet3 = player.GetItemCountByStoneVnum(self.GetVnumMaterial(i))
				
				adet_total = adet+adet2+adet3
				if adet_total >= self.GetCountMaterial(i):
					number = adet_total / (self.GetCountMaterial(i))
...
class CubeRenewalWindows(ui.ScriptWindow):
	...
	def __init__(self):
		...
		self.categoria = [
			[1,"SÝLAH2","Silah Dönüþümleri Efsanevi Seviye [PVP]"],
			[2,"SÝLAH4","Silah Dönüþümleri Efsanevi Seviye [PVM]"],
			[3,"ZIRH1","Zýrh Dönüþümleri Orta Seviye [PVP]"],
			[4,"ZIRH2","Zýrh Dönüþümleri Efsanevi Seviye [PVP]"],
			[5,"ZIRH3","Zýrh Dönüþümleri Orta Seviye [PVM]"],
			[6,"ZIRH4","Zýrh Dönüþümleri Efsanevi Seviye [PVM]"],
			[7,"KASK1","Kask Dönüþümleri Orta Seviye"],
			[8,"KASK2","Kask Dönüþümleri Efsanevi Seviye"],
			[9,"KOLYE1","Kolye Dönüþümleri Orta Seviye [PVP]"],
			[10,"KOLYE2","Kolye Dönüþümleri Efsanevi Seviye[PVP] "],
			[11,"BILEZIK1","Bilezik Dönüþümleri Orta Seviye [PVP]"],
			[12,"BILEZIK2","Bilezik Dönüþümleri Efsanevi Seviye [PVP]"],
			[13,"KUPE1","Küpe Dönüþümleri Orta Seviye [PVP]"],
			[14,"KUPE2","Küpe Dönüþümleri Efsanevi Seviye [PVP]"],
			[15,"AYAKKABI1","Ayakkabý Dönüþümleri Efsanevi Seviye [PVP]"],
			[16,"KALKAN1","Kalkan Dönüþümleri Efsanevi Seviye [pvp]"],
			[17,"KEMER1","Kemer Dönüþümleri Efsanevi Seviye [PVP]"],
			[18,"KEMER2","Kemer Dönüþümleri Efsanevi Seviye [PVM]"],
			[19,"FARM1","Farmlýk Taký Dönüþümleri Efsanevi Seviye [PVM]"],
			[20,"ELEMENTAL","Element Týlsýmý Dönüþümü Efsanevi Seviye"],
			[21,"RITUEL","Yükseltme Ýksiri Dönüþümleri"],
			[22,"EVENT","Etkinlik Dönüþümleri"],
			[23,"PERMA1","Pemalit Bekçi ve Madalyon Dönüþümleri"],
			[24,"PERMA","Permalit Taký Dönüþümleri [PVP]"],
			[25,"EVENT","Paskalya Etkinliði Yumurta Dönüþümü"],
			[26,"COIN","AlparCoin Ýþle"],
			[27,"COIN1","AlparCoin'i Ejderha Parasýna dönüþtür."],
			[28,"OTOBOT","Otomatik Metin ve Fam Botu Dönüþtür"],
			[29,"FARM3","Efsanevi Gökyüzü Takýlarý[PVM]"],
			[30,"BILET","Perma Bilet Bozdurma"],
			[999,"WORLDARD", "Vs."]
		]
	...
	def UpdateMaterialCount(self):
		...
		for i in xrange(1,6):
			if self.selectedItem.GetVnumMaterial(i) != 0:
				adet = player.GetItemCountByVnum(self.selectedItem.GetVnumMaterial(i))
				adet2 = player.GetItemCountByUpgradeVnum(self.selectedItem.GetVnumMaterial(i))
				adet3 = player.GetItemCountByStoneVnum(self.selectedItem.GetVnumMaterial(i))
				adet_total = adet+adet2+adet3
				if adet_total >= (self.selectedItem.GetCountMaterial(i)*count):
				...
				self.cube_elements["count_elements_%d"%i].SetText("%d/%d"%(adet_total,self.selectedItem.GetCountMaterial(i)*count))
				self.cube_elements["count_elements_%d"%i].Show()
			else:
				self.cube_elements["count_elements_%d"%i].Hide()

			count_line = len(constInfo.cube_count_items)
			constInfo.cube_count_items[count_line] = self.selectedItem.GetVnumMaterial(i)

		self.yang_text.SetText(localeInfo.NumberToMoneyString2(self.selectedItem.GetCheque()*count))
		self.yang_text.Show()
		self.gem_text.SetText(localeInfo.NumberToMoneyString2(self.selectedItem.GetGem()*count))
		self.gem_text.Show()
		self.gem2_text.SetText(localeInfo.NumberToMoneyString2(self.selectedItem.GetGem2()*count))
		self.gem2_text.Show()
