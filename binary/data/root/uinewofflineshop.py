import offlineshop, ui, app, item, player, dbg, uiCommon, uiToolTip, os
import localeInfo, ime, grp, snd, uiPickMoney, mouseModule, constInfo,pysettings

ENABLE_CHEQUE_SYSTEM = 1
YANG_PER_CHEQUE = 1000000
...
class ShopSearchItem(NewListBoxItem):
	...
	def LoadDialog(self):
		...
		self.itemGrid = ui.ExpandedImageBox()
		self.itemGrid.SetParent(self)
		self.itemGrid.SetPosition(2, ShopSearchItem.MARGIN)
		self.itemGrid.Show()

		self.RegisterComponent(self.itemGrid)

		self.itemImage = ui.ExpandedImageBox()
		self.itemImage.SetParent(self.itemGrid)
	...
	def SetSlot(self, itemvnum, itemcount):
		...
		itemCount = self.slotInfo["count"]
		item.SelectItem(iconVnum)		

		image 	= "ibrahimgokalp/offlineshop/tab_01.tga"
	...
	def __SetData(self, itemName, itemCount, itemWidth, itemHeight):
		...
		lSpace = 85
		priceYang = info["price"]
		priceCheque = info["cheque"]
		...
		if itemCount > 1:
			self.itemName.SetText(itemName + "|cffffb72d x " + "("+ str(itemCount)+")")
		else:
			lSpace = 100
			self.itemName.SetText(itemName)

		if itemHeight == 1:
			self.itemGrid.LoadImage("ibrahimgokalp/offlineshop_22/32x32.png")
		elif itemHeight == 2:
			self.itemGrid.LoadImage("ibrahimgokalp/offlineshop_22/32x64.png")
		else:
			self.itemGrid.LoadImage("ibrahimgokalp/offlineshop_22/32x96.png")

		self.itemPriceYang.SetText(localeInfo.NumberToMoneyStringArama(priceYang))
		self.itemPriceCheque.SetText(localeInfo.NumberToMoneyWonString(priceCheque))
		self.itemName.SetPosition(40, ShopSearchItem.MARGIN + (itemHeight * 32 - ShopSearchItem.COUNT_FONT_SIZE) / 2 - 10)
		self.itemOwner.SetPosition(40, ShopSearchItem.MARGIN + (itemHeight * 32 - ShopSearchItem.COUNT_FONT_SIZE) / 2 + 10)
		self.itemPriceYang.SetPosition(295, ShopSearchItem.MARGIN + (itemHeight * 32 - ShopSearchItem.COUNT_FONT_SIZE) / 2)
		self.itemPriceCheque.SetPosition(395, ShopSearchItem.MARGIN + (itemHeight * 32 - ShopSearchItem.COUNT_FONT_SIZE) / 2)
	...
	def __OverInItem(self):
		...
		if not self.tooltipItem.IsShow():
			...
			if ENABLE_CHEQUE_SYSTEM :
				self.tooltipItem.AppendSellingChequePrice(info["cheque"])
...
class NewOfflineShopBoard(ui.ScriptWindow):
	BOARD_KEYS = ("create_shop" , "my_shop", "open_shop", "search_history", )
	...
	def clear(self):
		...
		self.MyShopShopTotalCount = None
	...
	def __loadWindow(self):
		...
		self.MyShopShopTotalCount	= self.GetChild("MyShopShopTotalCount")
		...
		self.OwnerShopGauge			= self.GetChild("OwnerShop_Time_Gauge")
		...
		self.MyShopEditTitleButton.Hide()
		...
		self.OpenShopBackToListButton.Hide()
		self.__MakeOpenShopItemsTable()


		self.TitleBar = self.GetChild("TitleBar")
		self.Menu = self.GetChild("Menu")
		self.TitleName = self.GetChild("TitleName")
		self.OwnerShopPlayerName = self.GetChild("OwnerShopPlayerName")
		self.TitleBar.SetCloseEvent(self.Close)
		
		self.OwnerShopTitle = self.GetChild("OwnerShopTitle")
		...
		self.SearchFilterSubTypeComboBox = ui.ComboBoxImage(self.pageBoards["search_filter"], "d:/ymir work/ui/privatesearch/private_leftSlotImg.sub",21,95)
		...
		self.SearchFilterTypeComboBox = ui.ComboBoxImage(self.pageBoards["search_filter"], "d:/ymir work/ui/privatesearch/private_leftSlotImg.sub",21,75)
	...
	def Close(self):
		...
		pysettings.IBOYINEKAFAYIYICEKHADIBAKALIM = 0
	...
	def __MakeCreateShopItemsTable(self):
		...
		table = TableWindowWithScrollbar(582, 330, 12,3, TableWindowWithScrollbar.SCROLLBAR_VERTICAL)
		table.SetParent(board)
		table.SetPosition(16, 17)
	...
	def __MakeMyShopOffersTable(self):
		board = self.pageBoards["my_shop"]
		table = TableWindowWithScrollbar(582, 32, 1, 5, TableWindowWithScrollbar.SCROLLBAR_HORIZONTAL)
		table.SetParent(board)
		table.SetPosition(16, 17)
		table.Show()
	...
	def __MakeSearchFilterCheckBoxes(self):
		...
		race = {
			"warrior" 	: { "x" : 15, 	"y" : 189, 	},
			"assassin" 	: { "x" : 15+27, 	"y" : 189, 	},
			"sura" 		: { "x" : 15+54, 	"y" : 189, 	},
			"shaman" 	: { "x" : 15+81, 	"y" : 189, 	},
			"wolfman"	: { "x" : 15+108, 	"y" : 189, 	},
		}
		...
		for k, v in race.items():
			checkbox = ui.OffShopCheckBox({'base' : "ibrahimgokalp/offlineshop_22/%s_base.png"%k , 'tip' : "ibrahimgokalp/offlineshop_22/%s_tip.png"%k,})
			checkbox.SetParent(self.pageBoards["search_filter"])
			checkbox.SetPosition(v["x"] , v["y"])
			checkbox.Hide()
			checkbox.Enable()
	...
	def __MakeSearchFilterResultItemsTable(self):
		...
		ItemContainerListBox.SetSize(419, 350)
		ItemContainerListBox.SetPosition(163,28)
	...
	if ENABLE_CHEQUE_SYSTEM:
		def __OnPickValute(self, title, acceptEvent, maxValue, maxCheque, max=10):
		...
		def __OnPickValute(self, title, acceptEvent, maxValue, max=10):
			self.CommonPickValuteDlg.SetMax(max)
			self.CommonPickValuteDlg.SetTitleName(title)
			self.CommonPickValuteDlg.SetAcceptEvent(acceptEvent)
	...
	def ShopFilterResultItem_SetValue( self,  key, index, *args):
		if key in ( "id", "vnum", "count", "price", "owner", "owner_name", 'trans', 'cheque'):
	...
	def __SetTooltip(self,info):
		...
		if is_building and not info.get('sold', False):
			...
			self.itemTooltip.AppendSpace(10)
			self.itemTooltip.AppendTextLine("(|Eemoji/key_lclick|e" "Envantere Taşı" + ")", infocolor)
	...
	def __OnUpdateShopSafeboxPage(self):
		...
		self.TitleName.SetText(localeInfo.OFFLINESHOP_SHOP_SAFEBOXPAGE)
	...
	def RefreshMyShopPage(self):
		...
		self.MyShopShopTitle.SetText(localeInfo.OFFLINE_SHOP_DUKKANINADINEAGA+name + "  ")
		self.MyShopShopDuration.SetText("|Eemoji/clock|e " + localeInfo.LEFT_TIME +" : "+ GetDurationString(self.ShopOpenInfo["duration"]) + " / 3 Gün")
		self.MyShopShopTotalCount.SetText(localeInfo.OFFLINESHOP_ITEMS_COUNT_TEXT%self.ShopOpenInfo["count"])
		self.MyShopGauge.SetPercentage(self.ShopOpenInfo["duration"],3*24*60)
	...
	def RefreshOpenShopPage(self):
		...
		if '@' in name:
			...
			ownerName2 = name[:name.find('@')]
			ownerName = "|cffFFC700|Hmsg:%s|h%s|h|r" % (ownerName, ownerName)
			ownerName2 = "%s" % (ownerName2)
			name = name[name.find('@')+1:]
		
		self.OpenShopShopTitle.SetText("  " + localeInfo.OFFLINESHOP_ITEMS_COUNT_TEXT%self.ShopOpenInfo["count"])
		self.OpenShopShopDuration.SetText(localeInfo.LEFT_TIME +GetDurationString(duration))
		self.TitleName.SetText(ownerName2 + localeInfo.OFFLINESHOP_SHOP_TITLENAME)
		self.OwnerShopTitle.SetText(localeInfo.OFFLINE_SHOP_DUKKANINADINEAGA+name + "  ")
		self.OwnerShopPlayerName.SetText(localeInfo.OFFLINESHOP_SATICI+ownerName2)
		self.OwnerShopGauge.SetPercentage(self.ShopOpenInfo["duration"],3*24*60)
...
class AdminPageDialog(ui.ScriptWindow):
	...
	def __Load(self):
		self.__Load_LoadScript("uiscript/offlineshopadminpanel.py")
	...
	def __OnClickChangeMusicButton(self):
		pysettings.OFFLINESHOP_FUNCTION = 1
		self.sayfa = NewOfflineShopBoard()
		self.sayfa.Open()
		pysettings.IBOYINEKAFAYIYICEKHADIBAKALIM = 1
		self.Hide()


	def __OnClick2ChangeMusicButton(self):
		pysettings.OFFLINESHOP_FUNCTION = 2
		self.sayfa = NewOfflineShopBoard()
		self.sayfa.Open()
		pysettings.IBOYINEKAFAYIYICEKHADIBAKALIM = 1
		self.Hide()
