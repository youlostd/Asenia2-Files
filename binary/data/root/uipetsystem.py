...
class PetSystemMain(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		try:
			...
			pyScrLoader.LoadScriptFile(self, "UIScript/petinformationwindow_n.py")
		except:
			import exception as exception
			exception.Abort("petinformationwindow_n.LoadWindow.LoadObject")
	...
	def IncreaseSkill(self, slotIndex):
		try:
			...
			if (mouseModule.mouseController.isAttached()):
				...
				if (player.SLOT_TYPE_BOOK_INVENTORY != attachedSlotType and player.SLOT_TYPE_INVENTORY != attachedSlotType):
					return

				item.SelectItem(itemVnum)
				questionDialog = uiCommon.QuestionDialog()
				if (item.GetVnum() == 55030):
					questionDialog.SetText(localeInfo.PET_RESET_OPEN)
					questionDialog.SetAcceptEvent(ui.__mem_func__(self.AcceptDialogSkill2))
	...
	def AcceptDialogSkill2(self):
		net.SendChatPacket("/petresetskill %s %s" % (str(self.questionDialog.attachedSlotPos), str(self.questionDialog.slotIndex)))
		self.questionDialog.Close()
		return True
	...
	if (app.ENABLE_PET_ATTR_DETERMINE):
		...
		def __OnClickDetermineButton(self):
			...
			if (player.GetItemCountByVnum(55032) == 0):
		...
		def OnResultPetAttrChange(self):
...
class PetSystemMini(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		try:
			pyScrLoader = ui.PythonScriptLoader()
			pyScrLoader.LoadScriptFile(self, "UIScript/petminiinformationwindow_n.py")
		except:
			import exception as exception
			exception.Abort("petminiinformationwindow_n.LoadWindow.LoadObject")
	...
	def OnUpdate(self):
		if constInfo.PET_LEVEL == 60 and constInfo.PET_EVOLUTION == 0:
			self.pet_icon_slot_ani_img.Show()
		elif constInfo.PET_LEVEL == 80 and constInfo.PET_EVOLUTION == 1:
			self.pet_icon_slot_ani_img.Show()
		elif constInfo.PET_LEVEL == 120 and constInfo.PET_EVOLUTION == 2:
			self.pet_icon_slot_ani_img.Show()
		else:
			self.pet_icon_slot_ani_img.Hide()
		if True == self.expwind.IsIn():
