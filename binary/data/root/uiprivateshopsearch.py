...
class PrivateShopSeachWindow(ui.ScriptWindow):
	...
	JEWELRY_MASK_SUBTYPE_DIC = {
		...
		item.MASK_ITEM_SUBTYPE_JEWELRY_ITEM_ELEMENT : localeInfo.CATEGORY_JEWELRY_ITEM_ELEMENT,
	}
	...
	def Open(self,iscash):
		if self.isloded == 0:
			...
			print self.SerchItemSlotList.items()
	def Close(self):
		self.Hide()
		self.isloded = 0
	...
	def RefreshList(self):
		maxcount = privateShopSearch.GetPrivateShopSearchResultMaxCount()
		print 1
		print maxcount
		page = privateShopSearch.GetPrivateShopSearchResultPage()
		print 2
		for line, nameSlotList in self.SerchItemSlotList.items():
			print privateShopSearch.GetPrivateShopSearchResult(line)
			(itemname, sellername, itemCount, itemPrice, priceWon) = privateShopSearch.GetPrivateShopSearchResult(line)
			import chat
			...
			if "" == itemname:
				...
				#chat.AppendChat(1,"dbg itemName[%s] | sellerName[%s] | %d, %d, %d" % (itemname, sellername, itemCount, itemPrice, priceWon))
				nameSlotList[0].SetText(itemname)
				nameSlotList[1].SetText(sellername)
				nameSlotList[2].SetText(str(itemCount))
				nameSlotList[3].SetText(str(self.NumberToMoneyString(itemPrice)))
				nameSlotList[4].SetText(str(priceWon))
	...
	def Pagebutton(self,number):
		...
		privateShopSearch.GetPrivateShopSearchResultSelectPage(self.nowpagenumber)
		self.RefreshList()
		self.clearEffectEtc()
	...
	def lastnextbutton(self):
		maxsize = privateShopSearch.GetPrivateShopSearchResultMaxCount()
	...
	## 다음 페이지 점프
	def nextbutton(self):
		maxitemcount = privateShopSearch.GetPrivateShopSearchResultMaxCount()
	...
	def Search(self):
		...
		if mingold > 2000000000 - 1:
			mingold = 2000000000 - 1
			self.mingoldvalue.SetText(str(2000000000 - 1))
			
		if maxgold > 2000000000 - 1:
			maxgold = 2000000000 - 1
			self.maxgoldvalue.SetText(str(2000000000 - 1))
		
		if app.ENABLE_CHEQUE_SYSTEM:
			if len(self.minchequevalue.GetText()) <= 0:
				self.minchequevalue.SetText(str(0))
			if len(self.maxchequevalue.GetText()) <= 0:
				self.maxchequevalue.SetText(str(100000-1))
			
			mincheque = int(self.minchequevalue.GetText())
			maxcheque = int(self.maxchequevalue.GetText())
			if mincheque > 100000 - 1:
				mincheque = 100000 - 1
				self.minchequevalue.SetText(str(100000 - 1))
				
			if maxcheque > 100000 - 1:
				maxcheque = 100000 - 1
				self.maxchequevalue.SetText(str(100000 - 1))
			...
			print "job: %d Masktype: %d Masksub: %d minrefine: %d maxrefine: %d minlevel: %d maxlevel: %d mingold: %d maxgold: %d itemname: %s mincheque: %d maxcheque: %d" % (job,Masktype,Masksub,minrefine,maxrefine,minlevel,maxlevel,mingold,maxgold,itemname,mincheque,maxcheque)
		else:
	...
	def OnBuyAcceptEvent(self):
		if self.selectitemRealindex == -1:
			chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.PRIVATESHOPSEARCH_SELECTITEM)
			return
		net.SendPrivateShopSerchBuyItem(privateShopSearch.GetSearchItemShopVID(self.selectitemRealindex),privateShopSearch.GetSearchItemPos(self.selectitemRealindex))
		self.clearEffectEtc()
		self.Search()
		self.popup.Close()
		self.popup = None
	...
	def __SelectItem(self,arg):
		...
		## 타겟 이펙트 생성	
		chrvid = privateShopSearch.GetSearchItemShopVID(self.selectitemRealindex)
