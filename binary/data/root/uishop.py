...
class ShopDialog(ui.ScriptWindow):
	...
	def UnselectItemSlot(self, selectedSlotPos):
		...
		if app.ENABLE_REMOTE_SHOP:
			if shop.IsPrivateShop() and self.uzakmarket == False:
	...
	def AskBuyItem(self, slotPos):
		...
		if app.ENABLE_CHEQUE_SYSTEM and shop.IsPrivateShop():
			price_cheque = localeInfo.NumberToMoneyString(itemPriceCheque)
		...
		if app.ENABLE_CHEQUE_SYSTEM and shop.IsPrivateShop() and price_cheque > 0 and itemPriceType == 0:
			itemBuyQuestionDialog.SetText(localeInfo.DO_YOU_BUY_ITEM_CHEQUE(itemName, itemCount, price_cheque, price))
		else:
			# itemBuyQuestionDialog = uicommon.QuestionDialogShop()
			if app.PRICE_TYPE:
				if itemPriceType == 0:
				...
				itemBuyQuestionDialog.SetText(localeInfo.DO_YOU_BUY_ITEM(itemName, itemCount, localeInfo.NumberToGoldString(itemPrice)))
			# itemBuyQuestionDialog.SetText2(localeInfo.SATIN_ALMAK_ISTIYOR_MUSUN)
