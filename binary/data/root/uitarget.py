...
class TargetBoard(ui.ThinBoard):
	...
	if app.ENABLE_VIEW_TARGET_DECIMAL_HP:
		def SetHP(self, hpPercentage, iMinHP, iMaxHP):
			...
			if app.ENABLE_VIEW_TARGET_DECIMAL_HP:
				self.hpDecimal.SetText(localeInfo.NumberToString(max(iMinHP, 0)) + "/" + localeInfo.NumberToString(iMaxHP))
	...
	if app.ENABLE_MESSENGER_BLOCK:
		...
		def OnBlockRemove(self):
			messenger.RemoveBlock(constInfo.MESSENGER_KEY)
			net.SendMessengerBlockRemoveByVIDPacket(constInfo.MESSENGER_KEY, chr.GetNameByVID(self.vid))
