...
struct BlockCipherDetail: public BlockCipherAlgorithm
{
	...
	virtual SymmetricCipher* CreateEncoder(const CryptoPP::byte* key, size_t keylen, const CryptoPP::byte* iv) const
	{
		return new CTR_Mode<T>::Encryption(key, keylen, iv);
	}

	virtual SymmetricCipher* CreateDecoder(const CryptoPP::byte* key, size_t keylen, const CryptoPP::byte* iv) const
	{
