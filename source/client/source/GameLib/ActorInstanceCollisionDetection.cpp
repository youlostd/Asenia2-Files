...
// #ifdef ENABLE_STOP_COLISSION_GLOBAL	
	...
	// #ifdef ENABLE_STOP_COLLISION_PLAYER_OX
		...
		// {	
			// if (0 <= rVictim.GetRace() && rVictim.GetRace() <= 8) // Check if the victim through which pass over a player (change 7 with 8 if u have wolfman)
...
// #endif		
	...
	if (rVictim.GetRace() >= 101 && rVictim.GetRace() <= 1092)
		return FALSE;

	if (rVictim.GetRace() >= 1101 && rVictim.GetRace() <= 2596)
		return FALSE;

	if (rVictim.GetRace() >= 5001 && rVictim.GetRace() <= 3790)
		return FALSE;

	if (rVictim.GetRace() >= 3801 && rVictim.GetRace() <= 6051)
		return FALSE;

	if (rVictim.GetRace() >= 6101 && rVictim.GetRace() <= 6151)
		return FALSE;


	int iGecilenler[] = {9001,9002,9003,9004,9005,9006,20011,20091,20092,20093,20094,20095,30000,34011,34012,34004,34009,34016,34033,34034,34008,34007,20201,20202,20203,20204,20205,20206,20207,20208,20209,20210,20211,20212,20213,20214,20215,20216,20217,20218,20219,20220,20221,20222,20223,20224,20225,20226,20227,20228,20229,20230,20110,20111,20112,20113,20114,20115,20116,20117,20118,20120,20121,20122,20123,20124,20125,20219,20226,20227,20231,20232,20247,20248,20249,20250,20251,20252,20253,20254,20255,20256,20257,20258,34045,34046,34049,34050,34053,34054,34064,34065,34055,34056,34057,34058,34080,38081};
