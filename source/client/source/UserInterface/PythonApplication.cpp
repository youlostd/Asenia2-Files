...
#endif
///Yeni
#if defined(ENABLE_CLIENT_RESTRICTION)
#include <shlwapi.h>
...
void CPythonApplication::SkipRenderBuffering(DWORD dwSleepMSec)
{
	...
	extern "C" __declspec(dllexport) DWORD ac2(DWORD);
#endif
bool CPythonApplication::Process()
{
#ifdef URIEL_AC_VERIFY
	DWORD dwSecret = 0xcb896787;
	if (ac2(dwSecret) != (((dwSecret ^ (dwSecret * 2)) & 44) - 0xECD1B3))
		return false;
#endif
...
//////////////////////////////////client s???
	...
	if (std::string(PathFindFileName(szExeFileName)) != "metin2client.bin" 
		&& std::string(PathFindFileName(szExeFileName)) != "Alpar2.exe"){
	...
	//Yeni
	if (!CanOpenNewClient()){
		TraceError("Client Sınırımız 3 ! ");
		SET_EXCEPTION("Client Sınırımız 3 !");
		return false;
	}
#endif
	//Yeni
