...
void CPythonCharacterManager::EfektGG(BYTE iFlag){
	CEffectManager::Instance().EfektGG(iFlag == 1 ? true:false);
}
...
#ifdef OTOMATIK_AV
CInstanceBase * CPythonCharacterManager::OtomatikHedefBul(CInstanceBase * pkInstMain)
{
	if (!pkInstMain){ return nullptr; }
	float fMinUzaklik = 0.0f;
	CInstanceBase * gidecekInstance = NULL;
	int dVeriSayisi = 0;
	CPythonPlayer & rkPlayer = CPythonPlayer::Instance();
	TCharacterInstanceMap::iterator itor = m_kAliveInstMap.begin();
	const bool ifEq = rkPlayer.OtoAvSaldiriTipiAl();
	for (; itor != m_kAliveInstMap.end(); ++itor){
		CInstanceBase * iHedef = itor->second;
		if (!iHedef || iHedef == pkInstMain || iHedef->IsDead() /*|| iHedef->GetDistance(rkPlayer.OtoAvBaslangicKonumuAl()) > 3000.0f */){ continue; }
		if (ifEq){
			if (!iHedef->IsEnemy()){ continue; }
		}
		else{
			if (!iHedef->IsStone()){ continue; }
		}
		//if (!iHedef || iHedef == pkInstMain || !iHedef->IsEnemy() || iHedef->IsDead() /*|| iHedef->GetDistance(rkPlayer.OtoAvBaslangicKonumuAl()) > 3000.0f */){ continue; }
		/**********Hedefle aramda yurunmeyecek bir yer var mi kontrolu**********/
		bool engelVarDur = false;
		TPixelPosition hedefPos; iHedef->NEW_GetPixelPosition(&hedefPos);
		...
		if (engelVarDur){ continue; }
		//if (pkInstMain->CheckAdvancing()){ CPythonChat::Instance().AppendChat(1, "CheckAdvancing"); }
		/**********Hedefle aramda yurunmeyecek bir yer var mi kontrolu**********/
		float fDistance = pkInstMain->GetDistance(iHedef);
		if (dVeriSayisi == 0 && fDistance < OTOMATIK_AV_MAX_ODAK_MESAFESI){
			fMinUzaklik = pkInstMain->GetDistance(iHedef);	//Yakinimdaki hedefler ile mesafemi kontrol et dusuk var ise minimum uzaklik olarka yaz
			gidecekInstance = iHedef;
			dVeriSayisi++;
		}
		if (fDistance < fMinUzaklik && fDistance < OTOMATIK_AV_MAX_ODAK_MESAFESI){
			fMinUzaklik = fDistance;
			gidecekInstance = iHedef;
			dVeriSayisi++;
		}
	}
	return gidecekInstance ? gidecekInstance : nullptr;
}
#endif
