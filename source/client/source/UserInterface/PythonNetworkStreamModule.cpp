...
PyObject* netSendChestDropInfo(PyObject* poSelf, PyObject* poArgs)
{
	...
	BYTE window_type;
	if (!PyTuple_GetInteger(poArgs, 1, &window_type))
		return Py_BadArgument();

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendChestDropInfo(iSlotIndex, window_type);
...
PyObject* netSendItemUseAllPacket(PyObject* poSelf, PyObject* poArgs)
{
	TItemPos Cell;
	switch (PyTuple_Size(poArgs))
	{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendItemUsePacket(Cell, true);
	return Py_BuildNone();
}
...
PyObject* netSendItemSellPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendItemSellPacket(Cell, count);
	return Py_BuildNone();
}
...
PyObject* netSendSpecialMovePacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendItemMovePacket(TItemPos(type, sourceCell), TItemPos(INVENTORY, targetCell), (short)count);
	return Py_BuildNone();
}
#endif
...
{
	int iWon;
	if (!PyTuple_GetInteger(poArgs, 0, &iWon))
		return Py_BuildException();

	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendItemDropPacket(TItemPos(RESERVED_WINDOW, 0), 0, (DWORD) iWon);
	return Py_BuildNone();
}
...
PyObject* netSendChequeDropPacketNew(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendItemDropPacketNew(TItemPos (RESERVED_WINDOW, 0), 0, (DWORD) iWon, 0);
	return Py_BuildNone();
}
#endif
...
{
	int lMinPrice;
	if (!PyTuple_GetInteger(poArgs, 0, &lMinPrice))
		return Py_BuildException();

	char * pcItemName;
	if (!PyTuple_GetString(poArgs, 1, &pcItemName))
		return Py_BuildException();

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendShopSearchFindPacket(lMinPrice, pcItemName);
...
PyObject * netSendChangePriceOfflineShopItem(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (!PyTuple_GetInteger(poArgs, 2, &iPriceCheque))
		return Py_BuildException();


	CPythonNetworkStream & rkNetStream = CPythonNetworkStream::instance();

	rkNetStream.SendChangePriceOfflineShopItem(bPos, iPrice, iPriceCheque);
...
PyObject * netSendAddOfflineShopItem(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (!PyTuple_GetInteger(poArgs, 0, &tDisplayPos.window_type))
		return Py_BuildException();

	if (!PyTuple_GetInteger(poArgs, 1, &tDisplayPos.cell))
		return Py_BuildException();

	if (!PyTuple_GetInteger(poArgs, 2, &bPos))
		return Py_BuildException();
	...
	CPythonNetworkStream & rkNetStream = CPythonNetworkStream::instance();
	rkNetStream.SendAddOfflineShopItem(tDisplayPos, bPos, iPrice, iPriceCheque);
	return Py_BuildNone();
}
...
#ifdef YANG_LIMIT
	ULDWORD uElk = 0;
	if (!PyTuple_GetUnsignedLongLong(poArgs, 0, &uElk)){ return Py_BuildException(); }
#else
	int iElk;
	if (!PyTuple_GetInteger(poArgs, 0, &iElk)){ return Py_BuildException(); }
#endif

	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
#ifdef YANG_LIMIT
	rkNetStream.SendExchangeElkAddPacket(uElk);
#else
...
#ifdef ENABLE_CHEQUE_SYSTEM
PyObject* netSendExchangeWonAddPacket(PyObject* poSelf, PyObject* poArgs)
{
	int iWon;
	if (!PyTuple_GetInteger(poArgs, 0, &iWon))
		return Py_BuildException();
	...
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendExchangeWonAddPacket(iWon);
	return Py_BuildNone();
}
#endif
...
PyObject* netSendPrivateShopSearchInfo(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (!PyTuple_GetInteger(poArgs, 0, &iJob))
		return Py_BuildException();
	int iMasktype;
	if (!PyTuple_GetInteger(poArgs, 1, &iMasktype))
		return Py_BuildException();
	int iMasksub;
	if (!PyTuple_GetInteger(poArgs, 2, &iMasksub))
		return Py_BuildException();
	int iMinRefine;
	if (!PyTuple_GetInteger(poArgs, 3, &iMinRefine))
		return Py_BuildException();
	int iMaxRefine;
	if (!PyTuple_GetInteger(poArgs, 4, &iMaxRefine))
		return Py_BuildException();
	int iMinLevel;
	if (!PyTuple_GetInteger(poArgs, 5, &iMinLevel))
		return Py_BuildException();
	int iMaxLevel;
	if (!PyTuple_GetInteger(poArgs, 6, &iMaxLevel))
		return Py_BuildException();
	int iMinGold;
	...
	if (!PyTuple_GetInteger(poArgs, 8, &iMaxGold))
		return Py_BuildException();
	char * iItemName;
	if (!PyTuple_GetString(poArgs, 9, &iItemName))
		return Py_BuildException();
	int iMinCheque;
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendPrivateShopSearchInfo(iJob, iMasktype, iMasksub, iMinRefine, iMaxRefine, iMinLevel, iMaxLevel, iMinGold, iMaxGold, iItemName, iMinCheque, iMaxCheque);

	//TraceError("iJob: %d iMasktype: %d iMasksub: %d iMinRefine: %d iMaxRefine: %d iMinLevel: %d iMaxLevel: %d iMinGold: %d iMaxGold: %d iItemName: %s iMinCheque: %d iMaxCheque: %d", iJob, iMasktype, iMasksub, iMinRefine, iMaxRefine, iMinLevel, iMaxLevel, iMinGold, iMaxGold, iItemName, iMinCheque, iMaxCheque);
...
void initnet()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "SendItemUseAllPacket",				netSendItemUseAllPacket,				METH_VARARGS },
...
#ifdef ENABLE_CHEQUE_SYSTEM
		{ "SendExchangeWonAddPacket",			netSendExchangeWonAddPacket,			METH_VARARGS },
#endif
#ifdef ENABLE_WON_EXCHANGE
