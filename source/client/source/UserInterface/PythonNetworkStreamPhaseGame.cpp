...
#ifdef URIEL_ANTI_CHEAT
	kStatePacket.uX = kStatePacket.lX;
	kStatePacket.uY = kStatePacket.lY;

	if (!urielac::GetCharacterStatePos(&kStatePacket.uX, &kStatePacket.uY, 0x3879b817, 0x1825e5b4))
		return true;
#endif
...
#endif
		if (POINT_GAYA == PointChange.Type)
		{
			if (PointChange.amount > 0)
			{
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnPickGaya", Py_BuildValue("(i)", PointChange.amount));
			}
		}
		if (POINT_RED_GAYA == PointChange.Type)
		{
			if (PointChange.amount > 0)
			{
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnPickRedGame", Py_BuildValue("(i)", PointChange.amount));
			}
		}
	}
		...
		case SHOP_SUBHEADER_GC_START:
			{
				...
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "StartShop", Py_BuildValue("(ii)", dwVID, pShopStartPacket->uzak_market));
#else
...
#ifdef YANG_LIMIT
bool CPythonNetworkStream::SendExchangeElkAddPacket(ULDWORD elk)
#else
	...
	if (pInstPlayer && pInstTarget)
	{
		if (!pInstTarget->IsDead())
		{
			if (pInstTarget->IsPC() || pInstTarget->IsBuilding())
				...
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "SetHPTargetBoard", Py_BuildValue("(iiLLi)", TargetPacket.dwVID, TargetPacket.bHPPercent, TargetPacket.iMinHP, TargetPacket.iMaxHP, TargetPacket.bElement));
#else
...
#ifdef URIEL_ANTI_CHEAT
bool CPythonNetworkStream::SendAttackPacket(UINT uMotAttack, DWORD dwVIDVictim, DWORD dwVIDVictimVerify)
#else
...
#ifdef URIEL_ANTI_CHEAT
	kPacketAtk.dwVictimVIDVerify = dwVIDVictimVerify;
#endif
...
bool CPythonNetworkStream::SendWonExchangeSellPacket(int wValue)
{
...
bool CPythonNetworkStream::SendWonExchangeBuyPacket(int wValue)
{
	TPacketCGWonExchange kWonExchangePacket;
...
bool CPythonNetworkStream::RecvOfflineShopPacket()
{
	...
	switch (packet_shop.subheader)
	{
		...
		case SHOP_SUBHEADER_GC_REFRESH_MONEY:
		{
			...
			CPythonPlayer::Instance().SetCurrentOfflineShopMoney(pOfflineShopMoney->dwMoney, pOfflineShopMoney->dwCheque);
			break;
		}
...
#ifdef OTOMATIK_AV
	if (rkElement.dwType == CInstanceBase::NEW_AFFECT_OTO_AV){CPythonPlayer::Instance().OtoAvAffectDurum(true);}
#endif
...
bool CPythonNetworkStream::RecvAffectRemovePacket()
{
	...
	if (!Recv(sizeof(kAffectRemove), &kAffectRemove))
		return false;
#ifdef OTOMATIK_AV
	if (kAffectRemove.dwType == CInstanceBase::NEW_AFFECT_OTO_AV){ CPythonPlayer::Instance().OtoAvAffectDurum(false); }
#endif
...
#ifdef ENABLE_SHOW_CHEST_DROP
bool CPythonNetworkStream::SendChestDropInfo(WORD wInventoryCell, BYTE wInventoryType)
{
	TPacketCGChestDropInfo packet;
	packet.header = HEADER_CG_CHEST_DROP_INFO;
	packet.wInventoryCell = wInventoryCell;
	packet.wInventoryType = wInventoryType;
...
bool CPythonNetworkStream::RecvBossTracking()
{
	...
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_BEVIS_BOSS_TRACKING", Py_BuildValue("(iiiii)", p.dead_time, p.regen_time, p.channel, p.mob_vnum, p.map_index));
	return true;
}
#endif
...
static constexpr auto DiscordClientID = "816823907063562270"; // C++14 or lastest required for constexpr
void CPythonNetworkStream::Discord_Start()
{
	StartTime = time(0);
	DiscordEventHandlers handlers;
	memset(&handlers, 0, sizeof(handlers));
	Discord_Initialize(DiscordClientID, &handlers, 1, nullptr); // Set your application ID
...
void CPythonNetworkStream::Discord_Update(const bool ingame)
{
	...
	discordPresence.largeImageKey = "profil";
	discordPresence.largeImageText = "Alpar2";
	discordPresence.smallImageKey = "profil";
	discordPresence.smallImageText = "Alpar2 Malazgirt Premium Server";
...
bool CPythonNetworkStream::RecvGoldChange()
{
	...
	if (pInstance)
	{
		...
		{
			CPythonPlayer & rkPlayer = CPythonPlayer::Instance();
			rkPlayer.SetGold(goldChange.ldValue);

			if (goldChange.ldAmount > 0)
			{
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnPickMoney", Py_BuildValue("(K)", goldChange.ldAmount));
			}
...
bool CPythonNetworkStream::RecvKillStatistics()
{
	...
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "ReceiveKillStatisticsPacket", Py_BuildValue("(iiii)", 
