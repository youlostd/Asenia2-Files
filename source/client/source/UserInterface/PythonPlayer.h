...
class CPythonPlayer : public CSingleton<CPythonPlayer>, public IAbstractPlayer
{
...
#ifdef OTOMATIK_AV
			TOtoAvSlot aOtoAvSlot[OTOMATIK_AV_SLOT_MAX_NUM];
#endif
	...
	public:
...
#endif
		// OfflineShop
		void	SetCurrentOfflineShopMoney(DWORD llMoney, DWORD dwCheque);
		...
		void	OtoAvBeceriAta(bool gelenDurum){ otoAvBeceri = gelenDurum; }
		bool	OtoAvBeceriAl(){ return otoAvBeceri; }
		void	OtoAvIksirAta(bool gelenDurum){ otoAvIksir = gelenDurum; }
		bool	OtoAvIksirAl(){ return otoAvIksir; }
		void	OtoAvOdakAta(bool gelenDurum){ otoAvOdak = gelenDurum; }
		bool	OtoAvOdakAl(){ return otoAvOdak; }
		void	OtoAvSaldiriTipiAta(bool gelenDurum){ otoAvSaldiriTipi = gelenDurum; }	//0 Slot , 1 Metin
		bool	OtoAvSaldiriTipiAl(){ return otoAvSaldiriTipi; }
		void	OtoAvSlotaEkle(int otoAvSlotIndex, DWORD dwVnum,bool beceriMi);
		void	OtoAvSlotDSAta(int otoAvSlotIndex, DWORD dSuresi);
		void	OtoAvSlottanSil(int otoAvSlotIndex);
		TOtoAvSlot & OtoAvSlotData(int gSlotIndex);
		void	OtoAvSlotAl(DWORD dwSlotPos,DWORD* dwVnum,DWORD* dolumSuresi);
		void	OtoAvZamanlayici(bool beceriSlot);
		void	OtoAvSkillKullan(DWORD dwSlotIndex,long suanTime,int slotIndex);
		TPixelPosition OtoAvBaslangicKonumuAl(){ return otoAvBaslangicKonumu; }
		void	OtoAvRezerveKullan();/*0 krmizi 1 mavi*/
		bool	OtoAvEngelKontrol(CInstanceBase* pHedef, CInstanceBase* pAna);
		void	OtomatikAvSlotKontrol(DWORD slotIndex);
		void	OtoAvAffectDurum(bool gelenDurum){ otoAvAffect=gelenDurum; }
		bool	OtoAvAffectVarMi(){ return otoAvAffect; }
#endif
		...
		void	SetItemCount(TItemPos Cell, short byCount, bool bRefresh = true);
#else
		void	SetItemCount(TItemPos Cell, BYTE byCount);
#endif
		void	SetItemEvolution(TItemPos Cell, DWORD evolution, bool bRefresh = true);
		...
		DWORD	GetItemCountByAttrVnum(DWORD dwVnum);
		...
		DWORD	GetTargetVID();
		void OpenPrivateMessage(const char* c_szName);
	...
	protected:
		...
		bool	otoAvIksir;
		bool	otoAvOdak;
		bool	otoAvSaldiriTipi;
		long	zSonSn;
		long	kpSonMs;
		long	mpSonMs;
		long	ekSonMs;	//20.01.2019 06:15 artik kullanilmayacak galiba. Duruma gore degisebilir dursun simdilik. Bu yorumu sonra gorursen sil bu degiskeni.
		long	sonHedefBulMs;
