...
#ifdef ENABLE_PEVES_NEW_EMOTION
	EMOTION_SELFIE,
	EMOTION_DANCE_7,
	EMOTION_DOZE,
	EMOTION_EXERCISE,
...
PyObject * playerGetItemCountByAttrVnum(PyObject* poSelf, PyObject* poArgs)
{
	int ivnum;
	if (!PyTuple_GetInteger(poArgs, 0, &ivnum))
		return Py_BuildException();

	int ItemNum = CPythonPlayer::Instance().GetItemCountByAttrVnum(ivnum);
	return Py_BuildValue("i", ItemNum);
}
...
PyObject * playerGetItemLink(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (pPlayerItem && CItemManager::Instance().GetItemDataPointer(pPlayerItem->vnum, &pItemData))
	{
		...
		len = snprintf(itemlink, sizeof(itemlink), "item:%x:%x:%x:%x:%x:%x:%x:%x:%d",
				...
				pPlayerItem->alSockets[0], pPlayerItem->alSockets[1], pPlayerItem->alSockets[2], pPlayerItem->alSockets[3], pPlayerItem->alSockets[4], pPlayerItem->alSockets[5], pPlayerItem->evolution);

		for (int i = 0; i < ITEM_ATTRIBUTE_SLOT_MAX_NUM; ++i)
		{
			if (pPlayerItem->aAttr[i].bType != 9999)
			{
...
PyObject * playerCanDetach(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (pScrollItemData->GetValue(0) == AURA_CLEAN_ATTR_VALUE0)
	{
		...
		if (pPlayerItem)
		{
			if (pPlayerItem->alSockets[AURA_ABSORBED_SOCKET] > 0)
				return Py_BuildValue("i", DETACH_METIN_OK);
			else
				return Py_BuildValue("i", DETACH_METIN_CANT);
		}
...
PyObject * playerGetRebirt(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("b", CPythonPlayer::Instance().GetStatus(POINT_REBIRT));
}
#endif
...
PyObject * playerGetRedGaya(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("i", CPythonPlayer::Instance().GetStatus(POINT_RED_GAYA));
}

#ifdef ENABLE_GUVENLI_PC
PyObject * playerGetMACAdress(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("s", CPythonPlayer::Instance().GetMacAddress());
}
#endif
...
PyObject* playerOpenPrivateMessage(PyObject* poSelf, PyObject* poArgs)
{
	char* c_szName;
	...
	CPythonPlayer::Instance().OpenPrivateMessage(c_szName);
	return Py_BuildNone();
}
...
#ifdef OTOMATIK_AV
PyObject * playerOtoAvSlotaPotEkle(PyObject* poSelf, PyObject* poArgs){
	int slotIndex,dwVnum;
	if (!PyTuple_GetInteger(poArgs, 0, &slotIndex)){ return Py_BuildException(); }
	if (!PyTuple_GetInteger(poArgs, 1, &dwVnum)){ return Py_BuildException(); }
	CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
	rkPlayer.OtoAvSlotaEkle(slotIndex,dwVnum,false);
	return Py_BuildNone();
}
PyObject * playerOtoAvSlotaBeceriEkle(PyObject* poSelf, PyObject* poArgs){
	int slotIndex, dwVnum;
	if (!PyTuple_GetInteger(poArgs, 0, &slotIndex)){ return Py_BuildException(); }
	if (!PyTuple_GetInteger(poArgs, 1, &dwVnum)){ return Py_BuildException(); }
	CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
	rkPlayer.OtoAvSlotaEkle(slotIndex, dwVnum,true);
	return Py_BuildNone();
}
PyObject * playerOtoAvSlotDSAta(PyObject* poSelf, PyObject* poArgs){
	int slotIndex, dwVnum;
	if (!PyTuple_GetInteger(poArgs, 0, &slotIndex)){ return Py_BuildException(); }
	if (!PyTuple_GetInteger(poArgs, 1, &dwVnum)){ return Py_BuildException(); }
	CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
	rkPlayer.OtoAvSlotDSAta(slotIndex, dwVnum);
	return Py_BuildNone();
}
PyObject * playerOtoAvSlotAl(PyObject* poSelf, PyObject* poArgs){
	int iSlotIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iSlotIndex)){ return Py_BuildException(); }
	DWORD dwVnum,dolumSuresi;
	CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
	rkPlayer.OtoAvSlotAl(iSlotIndex, &dwVnum, &dolumSuresi);
	return Py_BuildValue("i", dwVnum);
}
PyObject * playerOtoAvSlotDolumSuresiAl(PyObject* poSelf, PyObject* poArgs){
	int iSlotIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iSlotIndex)){ return Py_BuildException(); }
	DWORD dwVnum, dolumSuresi;
	CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
	rkPlayer.OtoAvSlotAl(iSlotIndex, &dwVnum, &dolumSuresi);
	return Py_BuildValue("i", dolumSuresi);
}
PyObject * playerSlotDolumSuresiKontrol(PyObject* poSelf, PyObject* poArgs){
	int iSlotIndex, gelenVnum,gelenDS;
	if (!PyTuple_GetInteger(poArgs, 0, &iSlotIndex)){ return Py_BuildException(); }
	if (!PyTuple_GetInteger(poArgs, 1, &gelenVnum)){ return Py_BuildException(); }
	if (!PyTuple_GetInteger(poArgs, 2, &gelenDS)){ return Py_BuildException(); }
	DWORD dwVnum, dolumSuresi;
	CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
	rkPlayer.OtoAvSlotAl(iSlotIndex, &dwVnum, &dolumSuresi);
	if (dwVnum && dwVnum == gelenVnum){
		if (iSlotIndex>=0 && iSlotIndex < OTOMATIK_AV_SKILL_MAX_NUM){
			...
			if (skillIndex){
				...
				if (!CPythonSkill::Instance().GetSkillData(skillIndex, &c_pSkillData)){ TraceError("Beceri data alinamad?! slotIndex:%d",dwVnum);/**/ return Py_BuildException(); }
				int skillDS = (int)c_pSkillData->GetSkillCoolTime(skillDerece);
				if (!skillDS){ return Py_BuildException(); }
				return Py_BuildValue("i", gelenDS < skillDS ? skillDS : gelenDS);
			}
		}
		else{
			if (iSlotIndex < OTOMATIK_AV_SLOT_MAX_NUM){
				...
				return Py_BuildValue("i", rtDs<gelenDS ? gelenDS:rtDs);
			}
		}
	}
	return Py_BuildValue("i",0);
...
PyObject * playerOtoAvSaldiriAta(PyObject* poSelf, PyObject* poArgs){
	int gelenDurum;
	if (!PyTuple_GetInteger(poArgs, 0, &gelenDurum)){ return Py_BuildException(); }
	CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
	rkPlayer.OtoAvSaldiriAta(gelenDurum);
	return Py_BuildNone();
}
PyObject * playerOtoAvBeceriAta(PyObject* poSelf, PyObject* poArgs){
	int gelenDurum;
	if (!PyTuple_GetInteger(poArgs, 0, &gelenDurum)){ return Py_BuildException(); }
	CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
	rkPlayer.OtoAvBeceriAta(gelenDurum);
	return Py_BuildNone();
}
PyObject * playerOtoAvIksirAta(PyObject* poSelf, PyObject* poArgs){
	int gelenDurum;
	if (!PyTuple_GetInteger(poArgs, 0, &gelenDurum)){ return Py_BuildException(); }
	CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
	rkPlayer.OtoAvIksirAta(gelenDurum);
	return Py_BuildNone();
}
...
PyObject * playerOtoAvBeceriAl(PyObject* poSelf, PyObject* poArgs){
	int gelenDurum;
	if (!PyTuple_GetInteger(poArgs, 0, &gelenDurum)){ return Py_BuildException(); }
	CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
	return Py_BuildValue("i", rkPlayer.OtoAvBeceriAl());
}
PyObject * playerOtoAvIksirAl(PyObject* poSelf, PyObject* poArgs){
	int gelenDurum;
	if (!PyTuple_GetInteger(poArgs, 0, &gelenDurum)){ return Py_BuildException(); }
	CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
	return Py_BuildValue("i", rkPlayer.OtoAvIksirAl());
}
...
PyObject * playerOtoAvSlottanSil(PyObject* poSelf, PyObject* poArgs){
	int slotIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &slotIndex)){ return Py_BuildException(); }
	CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
	rkPlayer.OtoAvSlottanSil(slotIndex);
	return Py_BuildNone();
}
PyObject * playerOtoAvBaslayabilirMi(PyObject* poSelf, PyObject* poArgs){
	CInstanceBase * anaInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();
	if (anaInstance){
		CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
		return Py_BuildValue("i", rkPlayer.OtoAvAffectVarMi());
	}
	return Py_BuildValue("i", 0);
}
PyObject * playerOtoAvSaldiriTipi(PyObject* poSelf, PyObject* poArgs){
	int gelenDurum;
	if (!PyTuple_GetInteger(poArgs, 0, &gelenDurum)){ return Py_BuildException(); }
	CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
	rkPlayer.OtoAvSaldiriTipiAta(gelenDurum);
	return Py_BuildNone();
}
PyObject * playerOtoAvSaldiriTipiAl(PyObject* poSelf, PyObject* poArgs){
	int gelenDurum;
	if (!PyTuple_GetInteger(poArgs, 0, &gelenDurum)){ return Py_BuildException(); }
	CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
	return Py_BuildValue("i", rkPlayer.OtoAvSaldiriTipiAl());
}
#endif
...
void initPlayer()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetItemCountByAttrVnum",				playerGetItemCountByAttrVnum,				METH_VARARGS },
		...
		{ "GetCheque",					playerGetWon,						METH_VARARGS },
#endif
		...
		#ifdef ENABLE_REBIRT_SYSTEM
		{ "GetRebirt",					playerGetRebirt,					METH_VARARGS },
		#endif
		#ifdef ENABLE_GUVENLI_PC
		{ "GetMacAddress",				playerGetMACAdress,					METH_VARARGS },
		#endif
		{ "GetGaya",					playerGetGaya,					METH_VARARGS },
		{ "GetRedGaya",					playerGetRedGaya,					METH_VARARGS },

#ifdef ENABLE_SKILL_COLOR_SYSTEM
		{ "SetSkillColor",					playerSetSkillColor,				METH_VARARGS },
		{ "GetSkillColor",					playerGetSkillColor,				METH_VARARGS },
#endif
       	{ "OpenPrivateMessage", playerOpenPrivateMessage, METH_VARARGS },

#ifdef OTOMATIK_AV
		{ "OtoAvSlotAl", playerOtoAvSlotAl,METH_VARARGS},
		{ "OtoAvSlotaPotEkle", playerOtoAvSlotaPotEkle, METH_VARARGS },
		{ "OtoAvSlotaBeceriEkle", playerOtoAvSlotaBeceriEkle, METH_VARARGS },
		{ "OtoAvSlotDSAta", playerOtoAvSlotDSAta, METH_VARARGS },
		{ "OtoAvSlotDolumSuresiAl",playerOtoAvSlotDolumSuresiAl,METH_VARARGS },
		{ "OtoAvSaldiriAta", playerOtoAvSaldiriAta, METH_VARARGS },
		{ "OtoAvSaldiriAl", playerOtoAvSaldiriAl, METH_VARARGS },
		{ "OtoAvBeceriAta", playerOtoAvBeceriAta, METH_VARARGS },
		{ "OtoAvBeceriAl", playerOtoAvBeceriAl, METH_VARARGS },
		{ "OtoAvIksirAta", playerOtoAvIksirAta, METH_VARARGS },
		{ "OtoAvIksirAl", playerOtoAvIksirAl, METH_VARARGS },
		{ "OtoAvOdakAta", playerOtoAvOdakAta, METH_VARARGS },
		{ "OtoAvOdakAl", playerOtoAvOdakAl, METH_VARARGS },
		{ "SlotDolumSuresiKontrol", playerSlotDolumSuresiKontrol, METH_VARARGS },
		{ "OtoAvDurumAta", playerOtoAvDurumAta, METH_VARARGS },
		{ "OtoAvDurumAl", playerOtoAvDurumAl, METH_VARARGS },
		{ "OtoAvSlottanSil",playerOtoAvSlottanSil,METH_VARARGS },
		{ "OtoAvBaslayabilirMi", playerOtoAvBaslayabilirMi,METH_VARARGS },
		{ "OtoAvSaldiriTipi", playerOtoAvSaldiriTipi, METH_VARARGS },
		{ "OtoAvSaldiriTipiAl", playerOtoAvSaldiriTipiAl, METH_VARARGS },
#endif
...
#ifdef ENABLE_CHEQUE_SYSTEM
	PyModule_AddIntConstant(poModule, "WON",							POINT_CHEQUE);
#endif
	PyModule_AddIntConstant(poModule, "GAYA",							POINT_GAYA);
	PyModule_AddIntConstant(poModule, "RED_GAYA",							POINT_RED_GAYA);
...
#endif
	PyModule_AddIntConstant(poModule, "REFINE_TYPE_NORMAL",			REFINE_TYPE_NORMAL);
	PyModule_AddIntConstant(poModule, "REFINE_TYPE_PREMIUM",		REFINE_TYPE_NOT_USED1);
	PyModule_AddIntConstant(poModule, "REFINE_TYPE_SCROLL",			REFINE_TYPE_SCROLL);
	PyModule_AddIntConstant(poModule, "REFINE_TYPE_HYUNIRON",		REFINE_TYPE_HYUNIRON);
	PyModule_AddIntConstant(poModule, "REFINE_TYPE_MONEY_ONLY",		REFINE_TYPE_MONEY_ONLY);
	PyModule_AddIntConstant(poModule, "REFINE_TYPE_MUSIN",			REFINE_TYPE_MUSIN);
	PyModule_AddIntConstant(poModule, "REFINE_TYPE_BDRAGON",		REFINE_TYPE_BDRAGON);
	PyModule_AddIntConstant(poModule, "REFINE_TYPE_RITUALS_SCROLL",	REFINE_TYPE_RITUALS_SCROLL);
	#ifdef ENABLE_REBIRT_SYSTEM
	PyModule_AddIntConstant(poModule, "REBIRT",							POINT_REBIRT);
	#endif
#ifdef OTOMATIK_AV
	PyModule_AddIntConstant(poModule, "SLOT_TYPE_OTOMATIK_AV", SLOT_TYPE_OTOMATIK_AV);
	PyModule_AddIntConstant(poModule, "OTOMATIK_AV_SLOT_MAX_NUM", OTOMATIK_AV_SLOT_MAX_NUM);
	PyModule_AddIntConstant(poModule, "OTOMATIK_AV_SKILL_MAX_NUM", OTOMATIK_AV_SKILL_MAX_NUM);
	PyModule_AddIntConstant(poModule, "OTOMATIK_AV_ITEM_SLOT_START", OTOMATIK_AV_ITEM_SLOT_START);
	PyModule_AddIntConstant(poModule, "OTOMATIK_AV_ITEM_SLOT_END", OTOMATIK_AV_ITEM_SLOT_END);
#endif
}
