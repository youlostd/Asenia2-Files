...
#if defined(ENABLE_DEFAULT_PRIV)
	...
	if (pi->account_index == 0)
	{
		...
		if (res->uiNumRows == 0)
		{
			if (strcmp("000000", szSafeboxPassword))
			{
				...
				delete pSafebox;//memory leak fix
				delete pi;
				return;
			}
		}
...
void CClientManager::QUERY_ITEM_SAVE(CPeer * pkPeer, const char * c_pData)
{
	...
	if (p->window == SAFEBOX || p->window == MALL)
	{
		...
		snprintf(szQuery, sizeof(szQuery), 
			"REPLACE INTO item%s (id, owner_id, window, pos, count, vnum, evolution, socket0, socket1, socket2, socket3, socket4, socket5, "
...
#endif
			"VALUES(%u, %u, %d, %d, %u, %u, %d, %ld, %ld, %ld, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, "
#ifdef ENABLE_NEW_PET_SYSTEM
