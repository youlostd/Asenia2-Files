...
bool CClientManager::InitializeItemAttrTable()
{
	char query[4096];
	snprintf(query, sizeof(query),
			"SELECT apply, apply+0, prob, lv1, lv2, lv3, lv4, lv5, weapon, body, wrist, foots, neck, head, shield, ear, costume_hair, costume_body, costume_weapon, element, binek, glove FROM item_attr%s ORDER BY apply",
			GetTablePostfix());
	...
	while ((data = mysql_fetch_row(pRes->pSQLResult)))
	{
		...
		str_to_number(t.bMaxLevelBySet[ATTRIBUTE_SET_ELEMENT], data[col++]);
		str_to_number(t.bMaxLevelBySet[ATTRIBUTE_SET_BINEK], data[col++]);
		...
		sys_log(0, "ITEM_ATTR: %-20s %4lu { %3d %3d %3d %3d %3d } { %d %d %d %d %d %d %d }",
				...
				t.bMaxLevelBySet[ATTRIBUTE_SET_ELEMENT],
				t.bMaxLevelBySet[ATTRIBUTE_SET_BINEK]
#ifdef ENABLE_GLOVE_SYSTEM
...
bool CClientManager::InitializeItemRareTable()
{
	char query[4096];
	snprintf(query, sizeof(query),
			"SELECT apply, apply+0, prob, lv1, lv2, lv3, lv4, lv5, weapon, body, wrist, foots, neck, head, shield, ear, element, glove FROM item_attr_rare%s ORDER BY apply",
			GetTablePostfix());
	...
	for (itertype(m_vec_itemTable) it = m_vec_itemTable.begin(); it != m_vec_itemTable.end(); it++)
	{
		if (g_stLocaleNameColumn != "name")
		{
			...
			snprintf(query, sizeof(query),
				...
				t.aApplies[0].bType, t.aApplies[0].lValue, t.aApplies[1].bType, t.aApplies[1].lValue, t.aApplies[2].bType, t.aApplies[2].lValue, t.aApplies[3].bType, t.aApplies[3].lValue, t.aApplies[4].bType, t.aApplies[4].lValue,
