...
size_t CreatePlayerSaveQuery(char * pszQuery, size_t querySize, TPlayerTable * pkTab)
{
	...
	queryLen = snprintf(pszQuery, querySize,
		...
		pkTab->dungeonindex,
#endif
		pkTab->horse_appearance,
		pkTab->pet_appearance,
		pkTab->petvnum,
		pkTab->petslot
		#ifdef ENABLE_REBIRT_SYSTEM
		,pkTab->rebirt,
...
void CClientManager::QUERY_PLAYER_LOAD(CPeer * peer, DWORD dwHandle, TPlayerLoadPacket * packet)
{
	...
	if ((c = GetPlayerCache(packet->player_id)))
	{
		...
		{
			snprintf(szQuery, sizeof(szQuery), 
					"SELECT id,window+0,pos,count,vnum,evolution,socket0,socket1,socket2,socket3,socket4,socket5,attrtype0,attrvalue0,attrtype1,attrvalue1,attrtype2,attrvalue2,attrtype3,attrvalue3,attrtype4,attrvalue4,attrtype5,attrvalue5,attrtype6,attrvalue6 "
					...
					"FROM item%s WHERE owner_id=%d AND (window < %d or window = %d or window = %d or window = %d or window = %d or window = %d or window = %d)",
					GetTablePostfix(), pTab->id, SAFEBOX, DRAGON_SOUL_INVENTORY, UPGRADE_INVENTORY, STONE_INVENTORY, CHEST_INVENTORY, ATTR_INVENTORY, SWITCHBOT);
#else
	...
	else
	{
		...
		snprintf(queryStr, sizeof(queryStr),
				...
				"dungeonindex,"
#endif
				"horse_appearance,pet_appearance,petvnum,petslot,rebirt,gaya,red_gaya FROM player%s WHERE id=%d",
				GetTablePostfix(), packet->player_id);
...
#ifdef ENABLE_SPECIAL_STORAGE
				"FROM item%s WHERE owner_id=%d AND (window < %d or window = %d or window = %d or window = %d or window = %d or window = %d or window = %d)",
				GetTablePostfix(), packet->player_id, SAFEBOX, DRAGON_SOUL_INVENTORY, UPGRADE_INVENTORY, STONE_INVENTORY, CHEST_INVENTORY, ATTR_INVENTORY, SWITCHBOT);
#else
				"FROM item%s WHERE owner_id=%d AND (window < %d or window = %d)",
...
#endif
	str_to_number(pkTab->horse_appearance, row[col++]);
	str_to_number(pkTab->pet_appearance, row[col++]);
	str_to_number(pkTab->petvnum, row[col++]);
	str_to_number(pkTab->petslot, row[col++]);
	#ifdef ENABLE_REBIRT_SYSTEM
	str_to_number(pkTab->rebirt, row[col++]);
	#endif
	str_to_number(pkTab->gaya, row[col++]);
	str_to_number(pkTab->red_gaya, row[col++]);
...
void CClientManager::__RESULT_PLAYER_DELETE(CPeer *peer, SQLMsg* msg)
{
	...
	if (msg->Get() && msg->Get()->uiNumRows)
	{
		...
		GetTablePostfix(), pi->player_id, SAFEBOX, DRAGON_SOUL_INVENTORY, UPGRADE_INVENTORY, STONE_INVENTORY, CHEST_INVENTORY, ATTR_INVENTORY, SWITCHBOT);
#else
