		...
		if (NULL != ent)
		{
			if (true == ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (true == ch->IsPC() && false == ch->IsDead())
				{
					...
					dam += (dam / 100) * addPct;
					dam += (ch->GetMaxHP() / 100) * pct;

					ch->Damage(pAttacker, dam, DAMAGE_TYPE_ICE);

					sys_log(0, "BlueDragon: Breath to %s pct(%lld) dam(%lld) overlap(%d)", ch->GetName(), pct, dam, overlapDamageCount);
				}
			}
		}
	}
		...
		if (NULL != ent)
		{
			if (true == ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (true == ch->IsPC() && false == ch->IsDead())
				{
					...
					sys_log(0, "BlueDragon: WeakBreath to %s addPct(%lld) dam(%lld)", ch->GetName(), addPct, dam);
				}
			}
		}
	}
		...
		if (NULL != ent)
		{
			if (true == ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (true == ch->IsPC() && false == ch->IsDead())
				{
					...
					sys_log(0, "BlueDragon: EarthQuake to %s addPct(%lld) dam(%lld) sec(%d)", ch->GetName(), addPct, dam, sec);
