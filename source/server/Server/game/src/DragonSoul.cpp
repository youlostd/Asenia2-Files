...
#else
#ifdef ENABLE_EXTENDED_DS_INVENTORY
	return col_type* DRAGON_SOUL_STEP_MAX* DRAGON_SOUL_BOX_SIZE* DRAGON_SOUL_INVENTORY_PAGE_COUNT + row_type* DRAGON_SOUL_BOX_SIZE* DRAGON_SOUL_INVENTORY_PAGE_COUNT;
#else
...
bool DSManager::PullOut(LPCHARACTER ch, TItemPos DestCell, LPITEM& pItem, LPITEM pExtractor)
{
	...
	// 캐릭터의 용혼석 추출 및 추가 혹은 제거. 부산물 제공.
	{
		...
		if (bSuccess)
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("EJDERHA_TASI_CIKTI"));
			pItem->AddToCharacter(ch, DestCell);
			return true;
		}
		else
...
#ifdef YANG_LIMIT
	if (ch->GetGold() < static_cast<ULDWORD>(fee))
#else
	if (ch->GetGold() < fee)
#endif
	{
...
#ifdef YANG_LIMIT
	ch->GoldChange(static_cast<LDWORD>(-fee),"DOREFINEGRADE");
#else
...
#ifdef YANG_LIMIT
	ch->GoldChange(static_cast<LDWORD>(-fee), "DOREFINEGRADE 2");
#else
	ch->PointChange(POINT_GOLD, -fee);
#endif
	LPITEM pResult = NULL;
