...
void CNewPetActor::ItemCubeFeed(int type)
{
	for (int i = 0; i < 25; ++i) {
		if (m_dwpetslotitem[i] != -1) {
			...
			if (itemxp->GetID() == ITEM_MANAGER::instance().FindByVID(this->GetSummonItemVID())->GetID() || m_pkOwner->GetExchange() || m_pkOwner->GetMyShop() || m_pkOwner->GetShopOwner() || m_pkOwner->IsOpenSafebox() || m_pkOwner->IsCubeOpen() || m_pkOwner->IsSashCombinationOpen() || m_pkOwner->IsSashAbsorptionOpen())
				return;
			if (type == 1)
			{
				if ((itemxp->GetVnum() >= 55401 && itemxp->GetVnum() <= 55410) || (itemxp->GetVnum() >= 55701 && itemxp->GetVnum() <= 55710) || itemxp->GetVnum() == 55001 || itemxp->GetVnum() == 55101) {
...
// void CNewPetActor::SetArtis(int index, int val)
// {
	// if (index == 1)
		// m_dwArtis1 = val;
	// else if (index == 2)
...
void CNewPetActor::UpdateTime()
{
	...
	if (m_dwTimePet >= 60) {
		...
		if (pSummonItem != nullptr) {
			...
			// pSummonItem->SetForceAttribute(5, 1, m_dwArtis1);
...
void CNewPetActor::Unsummon()
{
	if (true == this->IsSummoned())
	{
		...
		if (pSummonItem != nullptr) {
			...
			std::unique_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("UPDATE new_petsystem SET level = %d, evolution=%d, exp=%d, expi=%d, bonus0=%d, bonus1=%d, bonus2=%d, skill0=%d, skill0lv= %d, skill1=%d, skill1lv= %d, skill2=%d, skill2lv= %d, duration=%d, tduration=%d WHERE id = '%lu' ", this->GetLevel(), this->m_dwevolution, this->GetExp(), this->GetExpI(), this->m_dwbonuspet[0][1], this->m_dwbonuspet[1][1], this->m_dwbonuspet[2][1], this->m_dwskillslot[0], this->m_dwskill[0], this->m_dwskillslot[1], this->m_dwskill[1], this->m_dwskillslot[2], this->m_dwskill[2], this->m_dwduration, this->m_dwtduration,/* this->m_dwArtis1, this->m_dwArtis2, this->m_dwArtis3,*/ ITEM_MANAGER::instance().FindByVID(this->GetSummonItemVID())->GetID()));
			this->ClearBuff();
			for (int b = 0; b < 3; b++) {
				pSummonItem->SetForceAttribute(b, 1, m_dwbonuspet[b][1]);
			}
...
DWORD CNewPetActor::Summon(const char* petName, LPITEM pSummonItem, bool bSpawnFar)
{
	...
	snprintf(szQuery1, sizeof(szQuery1), "SELECT name,level,exp,expi,bonus0,bonus1,bonus2,skill0,skill0lv,skill1,skill1lv,skill2,skill2lv,duration,tduration,evolution,ageduration,pet_type FROM new_petsystem WHERE id = '%d' ", pSummonItem->GetID());
...
bool CNewPetActor::_UpdatAloneActionAI(float fMinDist, float fMaxDist)
{
	...
	//m_pkChar->SetRotation(number(0, 359));        // 방향은 랜?으로 설정
...
bool CNewPetActor::_UpdateFollowAI()
{
	...
	// m_pkChar->m_pkMobData 값이 invalid한 경우가 자주 발?함. 현재 시간관계상 원인은 다음에 파악하고 일단은 m_pkChar->m_pkMobData 값을 아예 사용하지 않도록 함.
	// 여기서 매번 검사하는 이유는 최초 초기화 할 때 정상 값을 제대로 못얻어오는 경우도 있음.. -_-;; ?????????
...
void CNewPetActor::GiveBuff()
{
	...
	// 559 Affect NewPet
	int cbonus[3] = {(int)m_pkOwner->GetMaxHP(), (int)m_pkOwner->GetPoint(POINT_DEF_GRADE), (int)m_pkOwner->GetPoint(POINT_ATT_GRADE)};
	for (int i = 0; i < 3; ++i) {
	...
	if (GetLevel() > 81 && m_dwevolution == 3) {
		for (int s = 0; s < 3; s++) {
			...
			case 21: // patron avcisi
			case 22: //Can cekme
			case 23: //Tuy gibi
			case 24: //Mayalama ustasi
			case 25: //Kavrayis
...
bool CNewPetSystem::Update(DWORD deltaTime)
{
	...
	// 전혀 다른 값이라서-_-; 여기에 입력으로 들어오는 deltaTime은 의미가 없음??
...
// void CNewPetSystem::SetArtis(int index, int val)
// {
	// for (TNewPetActorMap::iterator iter = m_petActorMap.begin(); iter != m_petActorMap.end(); ++iter)
	// {
		// CNewPetActor* petActor = iter->second;
...
/// 등록 된 펫 중에서 주어진 몹 VNUM을 가진 액터를 ?환하는 함수.
