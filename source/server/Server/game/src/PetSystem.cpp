...
bool CPetActor::_UpdateFollowAI()
{
	...
	float	START_FOLLOW_DISTANCE = 100.0f;		// 이 거리 이상 떨어지면 쫓아가기 시작함
	float	START_RUN_DISTANCE = 300.0f;		// 이 거리 이상 떨어지면 뛰어서 쫓아감.

	float	RESPAWN_DISTANCE = 1500.f;			// 이 거리 이상 멀어지면 주인 옆으로 소환함.
