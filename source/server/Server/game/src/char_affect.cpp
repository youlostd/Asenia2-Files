...
#define IS_NO_CLEAR_ON_DEATH_AFFECT(type) ((type) == AFFECT_SHOP_DECO || (type) == AFFECT_BLOCK_CHAT || ((type) >= 500 && (type) < 750))
...
CAffect * CHARACTER::FindAffect2(DWORD dwType, BYTE bApply, long value) const
{
	/*if (dwType == 0)
		return NULL;*/
	...
	while (it != m_list_pkAffect.end())
	{
		...
		if (pkAffect->dwType == dwType && (bApply == APPLY_NONE || bApply == pkAffect->bApplyOn) && (pkAffect->lApplyValue == value))
			return pkAffect;
	}
...
bool CHARACTER::UpdateAffect()
{
	...
	if (GetPoint(POINT_HP_RECOVERY) > 0)
	{
		...
		else
		{
			int iVal = 0;
			if (LC_IsYMIR())
				iVal = MIN(GetPoint(POINT_HP_RECOVERY), (GetMaxHP() / 100) * 9);
			else
				iVal = MIN(GetPoint(POINT_HP_RECOVERY), (GetMaxHP() / 100) * 7);
	...
	if (GetPoint(POINT_SP_RECOVERY) > 0)
	{
		...
		{
			int iVal = MIN(GetPoint(POINT_SP_RECOVERY), (GetMaxSP() / 100) * 7);
...
void CHARACTER::ClearAffect(bool bSave)
{
	...
	while (it != m_list_pkAffect.end())
	{
		...
		if (bSave)
		{
			...
			if (pkAff->dwType == AFFECT_BONUSSET)
			{
				++it;
				continue;
			}
			...
			// {
				...
				// }
			// }
			if (pkAff->dwType >= AFFECT_POTION_1 && pkAff->dwType <= AFFECT_ENERGY)
			{
				++it;
				continue;
			}
			...
			if (pkAff->dwType >= AFFECT_YESIL && pkAff->dwType <= AFFECT_MOR)
			{
				++it;
				continue;
			}
			if (pkAff->dwType >= AFFECT_POTION_7 && pkAff->dwType <= AFFECT_POTION_9)
			{
				++it;
				continue;
			}
...
void CHARACTER::LoadAffect(DWORD dwCount, TPacketAffectElement * pElements)
{
	...
	for (DWORD i = 0; i < dwCount; ++i, ++pElements)
	{
		...
		for (int say = 0; say < _countof(listVnums2); say++)
		{
			if (pElements->dwType == listAffectPotion2[say])
			{
				LPITEM itembul = FindItemByID( pElements->dwFlag );
				
				
				if (NULL != itembul)
				{
				
					if (itembul->GetVnum() == listVnums2[say])
					{
						itembul->Lock(true);
					}
				}	
...
bool CHARACTER::AddAffect(DWORD dwType, BYTE bApplyOn, long lApplyValue, DWORD dwFlag, long lDuration, long lSPCost, bool bOverride, bool IsCube )
{
	...
	switch (dwType)
	{
...
#endif
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, "<AddAffect>(%s) Engellendi.Gelen buff etkisi(%ld%%)suanki buff etkisi (%ld%%).", CSkillManager::instance().Get(dwType)->szName, lApplyValue, pkAffect->lApplyValue);
				return false;
			}
		}
		break;
