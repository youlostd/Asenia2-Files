...
void CHARACTER::RewardGold(LPCHARACTER pkAttacker)
{
...

    {
        ...
        if (1 == number(1, iGold10DropPct))
            ...
            iGoldMultipler *= 10; // 1% C®·ü·Î µ· 10ac
...

        {
            ...
            // NOTE: µ· CrLþRþ Á¦ 3R?LO Al¸®¸¦ ?Áö lER
...
void CHARACTER::Reward(bool bItemDrop)
{
	...
	if (ITEM_MANAGER::instance().CreateDropItem(this, pkAttacker, s_vec_item))
	{
		...
		else if (s_vec_item.size() == 1)
		{
			...
			sys_log(0, "DROP_ITEM_TEST: %s %d %d from %s", item->GetName(), pos.x, pos.y, GetName());
		}
		else
		{
			...
			if (v.empty())
			{
				...
				while (iItemIdx >= 0)
				{
					...
					sys_log(0, "DROP_ITEM_TEST2: %s %d %d by %s", item->GetName(), pos.x, pos.y, GetName());
				}
			}
			else
			{
				...
				while (iItemIdx >= 0)
				{
					...
					sys_log(0, "DROP_ITEM_TEST3: %s %d %d by %s", item->GetName(), pos.x, pos.y, GetName());
				}
			}
		}
	}
...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
	...
	if (IsMonster())
	{
		if ((GetRaceNum() == TEMPLE_OCHAO_GUARDIAN) && (pkKiller && pkKiller->GetMapIndex() == TEMPLE_OCHAO_MAP_INDEX))
	...
	if (IsMonster())
	{
		if (!IsPC() && pkKiller && pkKiller->IsPC())
		{
			if (IsMonster() == true && quest::CQuestManager::instance().GetEventFlag("enable_boss_tracking") == 0)
			{
				...
				//if (CBossTracking::instance().RegenTimeGetNew(GetMobTable().dwVnum) != 0)
				{
					...
					if (pkRegen)
					{
						if (pkRegen->time != 0)
						{
							...
							StartBossTrackingDataEvent();
							StopBossTrackingDataEvent();
						}
					}
				}
			}
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int64_t dam, EDamageType type) // returns true if dead
{
	...
	if (GetRaceNum() == 5001)
	{
		...
		int64_t iPercent = (GetHP() / GetMaxHP()) * 100;
	...
	if (pAttacker)
	{
		...
		else if (GetRaceNum() == 6119 && CountBossByVnum(6151) > 0)
		{
			SendDamagePacket(pAttacker, 0, DAMAGE_BLOCK);
			return false;
		}
		else if (GetRaceNum() == 6119 && CountBossByVnum(8058) > 0)
		{
			SendDamagePacket(pAttacker, 0, DAMAGE_BLOCK);
			return false;
		}
		else if (GetRaceNum() == 6120 && CountBossByVnum(6051) > 0)
		{
			SendDamagePacket(pAttacker, 0, DAMAGE_BLOCK);
			return false;
		}
		else if (GetRaceNum() == 6120 && CountBossByVnum(8057) > 0)
		{
			SendDamagePacket(pAttacker, 0, DAMAGE_BLOCK);
			return false;
		}
		else if (GetRaceNum() == 6121 && CountBossByVnum(6408) > 0)
		{
			SendDamagePacket(pAttacker, 0, DAMAGE_BLOCK);
			return false;
		}
		else if (GetRaceNum() == 6121 && CountBossByVnum(8062) > 0)
		{
			SendDamagePacket(pAttacker, 0, DAMAGE_BLOCK);
			return false;
		}
		else if (GetRaceNum() == 6122 && CountBossByVnum(2597) > 0)
		{
			SendDamagePacket(pAttacker, 0, DAMAGE_BLOCK);
			return false;
		}
		else if (GetRaceNum() == 6122 && CountBossByVnum(8038) > 0)
		{
			SendDamagePacket(pAttacker, 0, DAMAGE_BLOCK);
			return false;
		}
		else if (GetRaceNum() == 6123 && CountBossByVnum(692) > 0)
		{
			SendDamagePacket(pAttacker, 0, DAMAGE_BLOCK);
			return false;
		}
		else if (GetRaceNum() == 6123 && CountBossByVnum(8116) > 0)
		{
			SendDamagePacket(pAttacker, 0, DAMAGE_BLOCK);
			return false;
		}
	...
	else if (type == DAMAGE_TYPE_NORMAL || type == DAMAGE_TYPE_NORMAL_RANGE)
	{
		...
		if (pAttacker)
		{
			...
			if (pAttacker->GetPoint(POINT_STEAL_HP))
			{
				...
				if (number(1, 10) <= pct)
				{
					int64_t iHP = (MIN(dam, MAX(0, iCurHP)) / 100) * pAttacker->GetPoint(POINT_STEAL_HP);
			...
			if (pAttacker->GetPoint(POINT_STEAL_SP))
			{
				...
				if (number(1, 10) <= pct)
				{
					...
					int64_t iSP = (MIN(dam, MAX(0, iCur)) / 100) * pAttacker->GetPoint(POINT_STEAL_SP);
...
#ifdef YANG_LIMIT
					pAttacker->GoldChange(static_cast<LDWORD>(iAmount),"GOLD_STEAL");
#else
			...
			// Ä¥ ¶§¸¶´Ù HPÈ¸º¹
			if (pAttacker->GetPoint(POINT_HIT_HP_RECOVERY) && number(0, 4) > 0) // 80% AŒ¢ç¡¤u
			{
				int64_t i = (MIN(dam, iCurHP) / 100) * pAttacker->GetPoint(POINT_HIT_HP_RECOVERY);
				if (i && i > 0)
				{
					CreateFly(FLY_HP_SMALL, pAttacker);
			...
			{
				int64_t i = (MIN(dam, iCurHP) / 100) * pAttacker->GetPoint(POINT_HIT_SP_RECOVERY);
	...
	if (pAttacker)
	{
		...
		if (test_server)
		{
			if(pAttacker)
			{
				pAttacker->ChatPacket(CHAT_TYPE_INFO, "-> %s, DAM %lld HP %lld(%lld%%) %s%s%s",
						GetName(), 
						dam, 
						GetHP(),
						(GetHP() / GetMaxHP()) * 100,
			...
			ChatPacket(CHAT_TYPE_PARTY, "<- %s, DAM %lld HP %lld(%lld%%) %s%s%s",
	...
	if (GetHP() <= 0)
	{
		if(pAttacker)
		{
			if(pAttacker->IsPC())
			{
				PointChange(POINT_HP_RECOVERY, 0);
				PointChange(POINT_SP_RECOVERY, 0);
				PointChange(POINT_HP, 0 - GetHP());
				Dead(pAttacker);
			}
			else
				Stun();
...
static void GiveExp(LPCHARACTER from, LPCHARACTER to, int iExp)
{
	...
	iExp = (iExp / 100) * (100 + CPrivManager::instance().GetPriv(to, PRIV_EXP_PCT));
	...
	if (LC_IsHongKong() || LC_IsEurope() || LC_IsCanada())
	{
		...
		{
			if (to->IsPCBang() == true)
				iExp += (iExp / 100) * to->GetPoint(POINT_PC_BANG_EXP_BONUS);
		}

		// °áÈ¥ º¸³Ê½º
		iExp += (iExp / 100) * to->GetMarriageBonus(UNIQUE_ITEM_MARRIAGE_EXP_BONUS);
	}
	...
	iExp += (iExp / 100) * to->GetPoint(POINT_RAMADAN_CANDY_BONUS_EXP);
	iExp += (iExp / 100) * to->GetPoint(POINT_MALL_EXPBONUS);
	iExp += (iExp / 100) * to->GetPoint(POINT_EXP);
...
*/
	...
	iExp = (iExp / 100) * CHARACTER_MANAGER::instance().GetMobExpRate(to);
	...
	if (test_server)
	{
		if (quest::CQuestManager::instance().GetEventFlag("exp_bonus_log") && iBaseExp > 0)
			to->ChatPacket(CHAT_TYPE_INFO, "exp bonus %d%%", ((iExp - iBaseExp) / iBaseExp) * 100);
	}
	...
	{
		if (to->GetNewPetSystem()->IsActivePet() && to->GetNewPetSystem()->GetLevelStep() < 4 && to->GetNewPetSystem()->GetLevel() < 120)
		{
			int tmpexp = (iExp / 20) * 9;
...

	{
		...
		{
			// 1¾ïÀÌ 100%
			DWORD dwUpdatePoint = (2000 / (2 * 3 * to->GetLevel())) * iExp;
...
LPCHARACTER CHARACTER::DistributeExp()
{
	...
	sys_log(1, "%s total exp: %d, damage_info_table.size() == %lld, TotalDam %lld",
			GetName(), iExpToDistribute, damage_info_table.size(), iTotalDam);
	//sys_log(1, "%s total exp: %d, pq_damage.size() == %d, TotalDam %lld",
...
void CHARACTER::UpdateAlignment(int iAmount)
{
	...
	m_iRealAlignment = MINMAX(-300000, m_iRealAlignment + iAmount, 1500000);
	AlignmentBonus();
