...
LPCHARACTER CHARACTER_MANAGER::SpawnMobRandomPosition(DWORD dwVnum, long lMapIndex)
{
	...
	if (pkMob->m_table.bType == CHAR_TYPE_NPC)
	{
		...
		if (ch->GetRaceNum() == 9003 || ch->GetRaceNum() == 9010 || ch->GetRaceNum() == 20381 || ch->GetRaceNum() == 9014 || ch->GetRaceNum() == 9016 || ch->GetRaceNum() == 9004 || ch->GetRaceNum() == 20504 || ch->GetRaceNum() == 20001 || ch->GetRaceNum() == 9001 || ch->GetRaceNum() == 9002 || ch->GetRaceNum() == 9013 || ch->GetRaceNum() ==9009 )
		{
			itertype(m_map_RemoteNpc) it = m_map_RemoteNpc.find(dwVnum);
			if (it == m_map_RemoteNpc.end())
				m_map_RemoteNpc.insert(std::make_pair(ch->GetRaceNum(), ch->GetVID()));
		}
	}
...
LPCHARACTER CHARACTER_MANAGER::SpawnMob(DWORD dwVnum, long lMapIndex, long x, long y, long z, bool bSpawnMotion, int iRot, bool bShow, bool isOfflineShopNPC, DWORD real_owner)
{
	...
	if (pkMob->m_table.bType == CHAR_TYPE_NPC)
	{
		...
		if (ch->GetRaceNum() == 9003 || ch->GetRaceNum() == 9010 || ch->GetRaceNum() == 20381 || ch->GetRaceNum() == 9014 || ch->GetRaceNum() == 9016 || ch->GetRaceNum() == 9004 || ch->GetRaceNum() == 20504 || ch->GetRaceNum() == 20001 || ch->GetRaceNum() == 9001 || ch->GetRaceNum() == 9002 || ch->GetRaceNum() == 9013 || ch->GetRaceNum() ==9009 )
		{
			// FN_Set_Shop_Mob(ch->GetRaceNum(), ch->GetVID());
			itertype(m_map_RemoteNpc) it = m_map_RemoteNpc.find(dwVnum);
			if (it == m_map_RemoteNpc.end())
				m_map_RemoteNpc.insert(std::make_pair(ch->GetRaceNum(), ch->GetVID()));
		}
...
void CHARACTER_MANAGER::DestroyCharacterInMap(long lMapIndex)
{
	...
	for (itertype(m_map_pkChrByVID) it = m_map_pkChrByVID.begin(); it != m_map_pkChrByVID.end(); it++)
	{
		...
		if (pkChr && pkChr->GetMapIndex() == lMapIndex && pkChr->IsNPC() && !pkChr->GetRaceNum() != 30000 && pkChr->GetRaceNum() != 30001 && pkChr->GetRaceNum() != 30002 && pkChr->GetRaceNum() != 30003 && pkChr->GetRaceNum() != 30004 && pkChr->GetRaceNum() != 30005 && pkChr->GetRaceNum() != 30006 && pkChr->GetRaceNum() != 30007 && !pkChr->IsPet()
#ifdef ENABLE_NEW_PET_SYSTEM
			 && !pkChr->IsNewPet()
#endif
			&& pkChr->GetRider() == NULL)
...
// DWORD CHARACTER_MANAGER::GetNpcVID(DWORD dwVnum)
// {
	// itertype(m_map_RemoteNpc) it = m_map_RemoteNpc.find(dwVnum);///eski uzak npc 
	// return (it == m_map_RemoteNpc.end()) ? 0 : it->second;
// }