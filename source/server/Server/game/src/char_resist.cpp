...
EVENTFUNC(poison_event)
{
	...
	int64_t dam = (ch->GetMaxHP() / 1000) * GetPoisonDamageRate(ch);
	if (test_server)
		ch->ChatPacket(CHAT_TYPE_NOTICE, "Poison Damage %lld", dam);
...
EVENTFUNC(fire_event)
{
	...
	if (test_server)
		ch->ChatPacket(CHAT_TYPE_NOTICE, "Fire Damage %lld", dam);
