...
void CHARACTER::SetSkillLevel(DWORD dwVnum, BYTE bLev)
{
	...
	if (dwVnum >= SKILL_ANTI_PALBANG && dwVnum <= SKILL_HELP_PASWAE)
#else
...
// #endif
#ifdef LWT_BUFFI_SYSTEM
	if (pkSkill->dwType != 245 || pkSkill->dwType != 246 || pkSkill->dwType != 247 || pkSkill->dwType != 248 || pkSkill->dwType != 249)
		return true;
#endif
	...
	{
#ifdef __WOLFMAN_CHARACTER__
		if (dwSkillVnum >= SKILL_ANTI_PALBANG && dwSkillVnum <= SKILL_HELP_PASWAE)
#else
...
#ifdef __7AND8TH_SKILLS__
#ifdef __WOLFMAN_CHARACTER__
	if (pkSk->dwVnum >= SKILL_ANTI_PALBANG && pkSk->dwVnum <= SKILL_HELP_PASWAE)
#else
	...
	switch (dwVnum)
	{
		case SKILL_ANTI_PALBANG:
		{
			if (GetSkillLevel(SKILL_ANTI_AMSEOP) == 0 &&
					...
					GetSkillLevel(SKILL_ANTI_GIGONGCHAM) == 0 &&GetSkillLevel(SKILL_ANTI_HWAJO) == 0 && GetSkillLevel(SKILL_ANTI_MAHWAN) == 0 && GetSkillLevel(SKILL_ANTI_BYEURAK) == 0
#ifdef __WOLFMAN_CHARACTER__
					&& GetSkillLevel(SKILL_ANTI_PASWAE) == 0
#endif
			)
				canLevelUP = true;
		...
		case SKILL_ANTI_AMSEOP:
		{
			if (GetSkillLevel(SKILL_ANTI_PALBANG) == 0 &&
					...
					GetSkillLevel(SKILL_ANTI_MAHWAN) == 0 &&
		...
		case SKILL_HELP_PALBANG:
		{
			if (GetSkillLevel(SKILL_HELP_AMSEOP) == 0 &&
					...
					GetSkillLevel(SKILL_HELP_MAHWAN) == 0 &&
					GetSkillLevel(SKILL_HELP_BYEURAK) == 0
#ifdef __WOLFMAN_CHARACTER__
					&& GetSkillLevel(SKILL_HELP_PASWAE) == 0
#endif
			)
				canLevelUP = true;
...
void SetPolyVarForAttack(LPCHARACTER ch, CSkillProto * pkSk, LPITEM pkWeapon)
{
    if (ch->IsPC())
    {
        ...
        else
        {
            ...
            pkSk->SetPointVar("mwep", 0 + ch->GetPoint(POINT_MAGIC_ATT_GRADE_BONUS));
        }
    }
    else
    {
	...
	void operator () (LPENTITY ent)
	{
...
#ifdef __7AND8TH_SKILLS__
		{
			...
			switch (m_pkSk->dwVnum)
			{
				...
				case SKILL_HWAJO:
					...
					break;
				case SKILL_MAHWAN:
					HELP_SKILL_ID = SKILL_HELP_MAHWAN;
					break;
				case SKILL_BYEURAK:
...
#ifdef __WOLFMAN_CHARACTER__
				case SKILL_PASWAE:
					HELP_SKILL_ID = SKILL_HELP_PASWAE;
					break;
#endif
				default:
					break;
		...
		
		{
			...
			switch (m_pkSk->dwVnum)
			{
				...
				case SKILL_HWAJO:
					...
					break;
				case SKILL_MAHWAN:
					ANTI_SKILL_ID = SKILL_ANTI_MAHWAN;
					break;
				case SKILL_BYEURAK:
...
#ifdef __WOLFMAN_CHARACTER__
				case SKILL_PASWAE:
					ANTI_SKILL_ID = SKILL_ANTI_PASWAE;
					break;
#endif
				default:
					break;
		...
		if (!pkChrVictim->Damage(m_pkChr, iDam, dt) && !pkChrVictim->IsStun())
		{
				...
				!IS_SET(pkChrVictim->GetAIFlag(), AIFLAG_NOMOVE))
			{
				...
				long tx = (long)(pkChrVictim->GetX());
				// long ty = (long)(pkChrVictim->GetY()+fy);
				long ty = (long)(pkChrVictim->GetY());
...
bool CHARACTER::UseSkill(DWORD dwVnum, LPCHARACTER pkVictim, bool bUseGrandMaster)
{
	...
	else if (IS_SET(pkSk->dwFlag, SKILL_FLAG_PARTY) && GetParty() && GetMapIndex() != 28)
	{
...
struct FHealerParty
{
	...
	void operator () (LPCHARACTER ch)
	{
		int64_t iRevive = (int64_t)(m_pkHealer->GetMaxHP() / 100 * 150);
		int64_t iHP = (ch->GetMaxHP() >= ch->GetHP() + iRevive) ? (int64_t)(ch->GetHP() + iRevive) : (int64_t)(ch->GetMaxHP());
		ch->SetHP(iHP);
		ch->EffectPacket(SE_EFFECT_HEALER);
		sys_log(0, "FHealerParty: %s (pointer: %p) heal the HP of %s (pointer: %p) with %lld (new HP: %lld).", m_pkHealer->GetName(), get_pointer(m_pkHealer), ch->GetName(), get_pointer(ch), iRevive, ch->GetHP());
	}
...
bool CHARACTER::UseMobSkill(unsigned int idx)
{
	...
	if ((IsMonster()) && (pkSk->dwVnum == HEALING_SKILL_VNUM))
	{
		...
		{
			int64_t iRevive = (int64_t)(GetMaxHP() / 100 * 150);
			int64_t iHP = (GetMaxHP() >= GetHP() + iRevive) ? (int64_t)(GetHP() + iRevive) : (int64_t)(GetMaxHP());
			SetHP(iHP);
			EffectPacket(SE_EFFECT_HEALER);
			sys_log(0, "FHealer: %s (pointer: %p) heal their HP with %lld (new HP: %lld).", GetName(), get_pointer(this), iRevive, GetHP());
		}
...
bool CHARACTER::CheckSkillHitCount(const BYTE SkillID, const VID TargetVID)
{
	...
	if (rSkillUseInfo.TargetVIDMap.end() != iterTargetMap)
	{
		...
		switch (SkillID)
		{
			...
			case SKILL_CHARYUN:
			case SKILL_CHAYEOL:
			//case SKILL_PASWAE:(kurt pencesi x3)
