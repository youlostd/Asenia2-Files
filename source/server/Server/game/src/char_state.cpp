...
namespace
{
	...
	class FuncFindHydraVictim
	{
		public:
			FuncFindHydraVictim(LPCHARACTER pkChr, int iMaxDistance) :
				m_pkChr(pkChr),
			m_iMinDistance(INT_MAX),
			m_iMaxDistance(iMaxDistance),
			m_lx(pkChr->GetX()),
	...
	class FuncFindMeleyVictim
	{
		public:
			FuncFindMeleyVictim(LPCHARACTER pkChr, int iMaxDistance) :
				m_pkChr(pkChr),
			m_iMinDistance(INT_MAX),
			m_iMaxDistance(iMaxDistance),
			m_lx(pkChr->GetX()),
...
void CHARACTER::__StateIdle_Monster()
{
	...
	if (GetRaceNum() >= 3950 && GetRaceNum() <= 3970)
	{
		
		FuncFindHydraVictim f(this, 50000);

		if (GetSectree())
			GetSectree()->ForEachAround(f);

		LPCHARACTER victim2 = f.GetVictim();
		if (victim2)
		{
	...
	if (GetRaceNum() >= 6110 && GetRaceNum() <= 6114)
	{
		
		FuncFindMeleyVictim f(this, 50000);
