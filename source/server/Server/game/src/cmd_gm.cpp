...
struct FuncPurge
{
	...
	void operator () (LPENTITY ent)
	{
		...
		//if (pkChr->IsNPC() && !pkChr->IsPet() && !pkChr->IsOfflineShopNPC() && pkChr->GetRaceNum() != 30000 || pkChr->GetRaceNum() != 30001 || pkChr->GetRaceNum() != 30002 || pkChr->GetRaceNum() != 30003|| pkChr->GetRaceNum() != 30004|| pkChr->GetRaceNum() != 30005 || pkChr->GetRaceNum() != 30006 || pkChr->GetRaceNum() != 30007 && pkChr->GetRider() == NULL)
		if (!pkChr->IsOfflineShopNPC() && pkChr->GetRaceNum() != 30000 &&  pkChr->GetRaceNum() != 30001 && pkChr->GetRaceNum() != 30002 && pkChr->GetRaceNum() != 30003 && pkChr->GetRaceNum() != 30004 && pkChr->GetRaceNum() != 30005 && pkChr->GetRaceNum() != 30006 && pkChr->GetRaceNum() != 30007 && pkChr->IsNPC() && !pkChr->IsPet() && pkChr->GetRider() == NULL
...
/* #ifdef LWT_BUFFI_SYSTEM
	...
	{
	ch->ChatPacket(CHAT_TYPE_INFO, "Yard?c??g?dermeden bu iþlemi yapamazs?. ");
	return ;
	}
#endif */
...
ACMD(do_state)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "   Monster : %d - Monster Resist : %d ",
			tch->GetPoint(POINT_ATT_MONSTER_NEW),
...
"Rizaveta",
	...
	const char* hazircevap[] = { "as", "a.s", "AS", ":as", "Aleykumselam","ase","aleykum selam","a.s eyvallah",};
	const char* kingdoms[3] = { "|Eemoji/kirmizibayrak|e  ","|Eemoji/saribayrak|e  ","|Eemoji/mavibayrak|e  " };
	...
	if (*pCount < (_countof(gm_change_name)) && *pRepeat > 0)
	{
		...
		snprintf(chatbuf, sizeof(chatbuf), "%s : %s", kim, giden);
...
ACMD(do_set)
{
	...
	switch (i)
	{
		case 0:	// gold
			{
				...
				ULDWORD gold = 0;
				str_to_number(gold, arg3);
				if (tch->GetGold() + gold >= tch->GetAllowedGold())
#else
				int gold = 0;
				str_to_number(gold, arg3);
				if (tch->GetGold() + gold >= GOLD_MAX)
#endif
				{
					ch->ChatPacket(CHAT_TYPE_INFO,"Bir oyuncuda en fazla 2 milyar Yang olabilir.");
					return;
				}
			
#ifdef YANG_LIMIT
				tch->GoldChange(static_cast<LDWORD>(gold), "GM_GIVE");
#else
				tch->PointChange(POINT_GOLD, gold, true);
#endif
			}
		...
		case 9: //Rebirt
			{
				int rebirt = 0;
				str_to_number(rebirt, arg3);
				int before_rebirt = tch->GetRebirt();
				tch->PointChange(POINT_REBIRT, rebirt, true);
				int after_rebirt = tch->GetRebirt();
				if (0 == after_rebirt && 0 != before_rebirt)
				{
					LogManager::instance().CharLog(tch, rebirt, "ZERO_REBIRT", "GM");
				}
			}
			break;
		#endif
		...
		case 'c':
			{
...
#ifdef YANG_LIMIT
					if (ch->GetGold() < static_cast<ULDWORD>(t->dwPrice))
#else
					...
					// °Ç¼³ Àç·á ¼Ò¸ðÇÏ±â (Å×¼·¿¡¼­´Â GMµµ ¼Ò¸ð)
				{
					// °Ç¼³ ºñ¿ë ¼Ò¸ð
#ifdef YANG_LIMIT
					LDWORD dwPrice = static_cast<LDWORD>(t->dwPrice);
					ch->GoldChange(-dwPrice, "BUILD");
#else
...
ACMD(do_kick_offline_shop)
{
	...
	if (pMap)
	{
		...
		if (offlineShop.bIsDestroyed)
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, "%s destroyed!", szName);
		}
		else
			ch->ChatPacket(CHAT_TYPE_INFO, "You can't destroy %s!", szName);
	}
}
ACMD(do_free_regen)
{
