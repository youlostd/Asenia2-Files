		...
		case QID_AUTH_LOGIN:
			{
				...
				else
				{
...
#ifdef OTOMATIK_AV
						str_to_number(aiPremiumTimes[PREMIUM_OTOMATIK_AV], row[col++]);
#endif
					}
					else
					{
					...
					else if (!strcmp(szStatus, "MACBAN"))
					{
						LoginFailure(d, "MACBAN");
						sys_log(0, "   MACBAN");
						M2_DELETE(pinfo);
					}
					else if (bNotAvail)
					...
					else
					{
						...
						char szQueryhwid[1024];
						snprintf(szQueryhwid, sizeof(szQueryhwid), "UPDATE account SET mac='%s', last_play=NOW() WHERE id=%u", szHwid, dwID);
						std::unique_ptr<SQLMsg> msg( DBManager::instance().DirectQuery(szQueryhwid) );
						
						std::unique_ptr<SQLMsg> _hwid_msg(DBManager::instance().DirectQuery("SELECT hwid FROM account.yasaklimac"));
						bool var = false;
