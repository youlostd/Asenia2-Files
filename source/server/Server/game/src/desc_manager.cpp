...
bool DESC_MANAGER::GuvenliPcKontrol(const char * login, const char * cHWInfo)
{

	/*if (cHWInfo == NULL)
	{
		sys_log(0, "cHWInfo null");
		return false;
	}
	...
	{
		sys_log(0, "inject engel sistemi: cHWInfo: %s", cHWInfo);
		return false;
	}*/
	...
	std::unique_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT guvenli_pc FROM player.guvenlipc WHERE login = '%s' and durum = 1", szLogin));
	if (!pMsg->uiSQLErrno)
	{
		if (pMsg->Get()->uiNumRows)
		{
			while (MYSQL_ROW row_guvenlipc = mysql_fetch_row(pMsg->Get()->pSQLResult))
			{
				if (strcmp(row_guvenlipc[0], cHWInfo))
				{
					return true;
				}
			}
