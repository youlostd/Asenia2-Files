...
int64_t CDungeon::GetUniqueMaxHp(const std::string& key)
{
	TUniqueMobMap::iterator it = m_map_UniqueMob.find(key);
	if (it == m_map_UniqueMob.end())
	{
...
float CDungeon::GetUniqueHpPerc(const std::string& key)
{
	...
	return (it->second->GetHP() / it->second->GetMaxHP()) * 100.f;
}

int64_t CDungeon::GetUniqueHp(const std::string& key)
{
	TUniqueMobMap::iterator it = m_map_UniqueMob.find(key);
	if (it == m_map_UniqueMob.end())
	{
	...
	struct FKillSectree
	{
		void operator () (LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (!ch->IsPC() && !ch->IsPet() && !ch->GetRider()&& !ch->IsSupport()
...
struct FCountMonster
{
	...
	void operator()(LPENTITY ent)
	{
		...
		{
			LPCHARACTER ch = (LPCHARACTER) ent;
			if (!ch->IsPC() && !ch->IsPet() && !ch->GetRider()
#ifdef ENABLE_NEW_PET_SYSTEM
				 && !ch->IsNewPet()
#endif
			)
...
struct FCountStone
{
	...
	void operator()(LPENTITY ent)
	{
		...
		{
			LPCHARACTER ch = (LPCHARACTER) ent;
			if (ch->IsStone() && ch->GetRaceNum() == vnum)
				n++;
		}
	}
};

int CDungeon::CountStoneByVnum(int vnum)
{
	LPSECTREE_MAP pMap = SECTREE_MANAGER::instance().GetMap(m_lMapIndex);
	if (!pMap)
	{
