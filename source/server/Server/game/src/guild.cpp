...
#ifdef YANG_LIMIT
	ch->GoldChange(static_cast<LDWORD>(-gold),"CHARGE_SP");
#else
...
#ifdef YANG_LIMIT
	if (ch->GetGold() < static_cast<ULDWORD>(iGold)) { return; }
	ch->GoldChange(static_cast<LDWORD>(-iGold),"DEPOSIT");
#else
	if (ch->GetGold() < iGold) { return; }
...
void CGuild::RecvWithdrawMoneyGive(int iChangeGold)
{
	...
	{
#ifdef YANG_LIMIT
		ch->GoldChange(static_cast<LDWORD>(iChangeGold),"GUILD");
#else
