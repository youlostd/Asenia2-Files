...
void CInputAuth::Login(LPDESC d, const char * c_pData)
{
	...
	unsigned long a = 458187797, b = 2134171267;
	...
	if (!pinfo->cHWInfo || pinfo->cHWInfo == NULL)
	{
		sys_log(0, "GUV_PC_MAC_YOK: [%s]", pinfo->login);
		LoginFailure(d, "MAC_YOK");
		return;
	}

	if (DESC_MANAGER::instance().GuvenliPcKontrol(pinfo->login, pinfo->cHWInfo))
	{
		sys_log(0, "GUV_PC: [%s] Mac: [%s]", pinfo->login, pinfo->cHWInfo);
		LoginFailure(d, "GUV_PC");
		return;
	}
	#endif
