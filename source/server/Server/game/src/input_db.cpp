	...
	if (g_bEnableRespawnOfflineShop)
	{
		...
		if(pMsg->Get()->uiNumRows != 0)
		{
			while (NULL != (row = mysql_fetch_row(pMsg->Get()->pSQLResult)))
			{
				...
				//Shop Decoration
				str_to_number(mNpc, row[12]);
				snprintf(mDecoration, sizeof(mDecoration), "%s", row[13]);
				//Shop Decoration
				...
				if (g_bChannel == bChannel)
				{
					...
					if (npc)
					{
						...
						std::string m_variable11 = m_variable00+szSign;
						//Shop Decoration
						npc->SetOfflineShopSign(m_variable11.c_str());
