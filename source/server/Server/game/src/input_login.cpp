...
#include "dev_log.h"
#include "log.h"
#ifdef ENABLE_NEW_LOGIN_LEVEL
	extern int BaslangicLvl;
#endif
...
{
	char buf[512];
	snprintf(buf, sizeof(buf), "RecvGameConfig %d %d %d",
...
#ifdef ENABLE_NEW_LOGIN_LEVEL
			table->level		= BaslangicLvl;
#else
			table->level		= 1;
#endif
	table->stat_point		+= (BaslangicLvl * 3) - 3;
	table->job = job;
	table->voice = 0;
...
void CInputLogin::Entergame(LPDESC d, const char * data)
{
	...
	if (!(ch->IsHorseRiding()))
	{
		bool iSummonHorse = (ch->GetHorseAppearance() != 0);
		if (iSummonHorse && ch->GetWarMap())
			iSummonHorse = false;

		if (iSummonHorse)
		{
			LPITEM binekvnum = ch->GetWear(WEAR_COSTUME_MOUNT);
			if (NULL != binekvnum)
			{
				if (ch->GetWarMap() == false)
				{
					ch->HorseSummon(true);
				}
			}
	...
	if (ch->GetPetAppearance() != 0)
	{
		LPITEM petvnum1 = ch->GetWear(WEAR_PET);
		if (NULL != petvnum1)
		{
			ch->PetGetir();
		}
	...
	{
		char infover[100];
		snprintf(infover, sizeof(infover), "%s", Takip[i][2]);
		int gonder = ch->GetQuestFlag(infover);
		ch->ChatPacket(CHAT_TYPE_COMMAND, "takipinfo %d %d", i, gonder);
	}
	...
	if (biodurum == 0 && ch->GetLevel() >= 30)
	{
		...
		ch->ChatPacket(CHAT_TYPE_COMMAND, "biyolog %d %d %d %d %d %d %d ", BiyologSistemi[biodurum][0], bioverilen, BiyologSistemi[biodurum][1], biokalan, BiyologSistemi[biodurum][4],BiyologSistemi[biodurum][5],bioruhtasi);
	...
	else
	{
		...
		if (biodurum == 1)
		{
			int lv30 = ch->GetQuestFlag("bio.30");
			int lv40 = ch->GetQuestFlag("bio.40");
			int lv50 = ch->GetQuestFlag("bio.50");
			int lv60 = ch->GetQuestFlag("bio.60");
			int lv70 = ch->GetQuestFlag("bio.70");
			int lv80 = ch->GetQuestFlag("bio.80");
			int lv85 = ch->GetQuestFlag("bio.85");
			int lv90 = ch->GetQuestFlag("bio.90");
			int lv92 = ch->GetQuestFlag("bio.92");
			int lv94 = ch->GetQuestFlag("bio.94");
			int lv100 = ch->GetQuestFlag("bio.100");
			int lv102 = ch->GetQuestFlag("bio.102");
			int lv105 = ch->GetQuestFlag("bio.105");
			int lv108 = ch->GetQuestFlag("bio.108");
			int lv110 = ch->GetQuestFlag("bio.110");
			...
			if (isbugged == true)
			{
				...
				ch->SetQuestFlag("bio.durum",biodurum);
			}
		}
		
		
		if(biodurum == 31 || biodurum == 41 || biodurum == 51 || biodurum == 61 || biodurum == 71 || biodurum == 81 || biodurum == 86 || biodurum == 91 || biodurum == 93 || biodurum == 100 || biodurum == 102 || biodurum == 105 || biodurum == 108 || biodurum == 110 || biodurum == 120 || biodurum == 125)
		{
			ch->ChatPacket(CHAT_TYPE_COMMAND, "biyolog 0 0 0 0 0 0 0");
		}
		else if (bioverilen >= BiyologSistemi[biodurum][1] && bioruhtasi == 1)
		{
			ch->ChatPacket(CHAT_TYPE_COMMAND, "biyolog %d 0 1 0 %d %d %d", BiyologSistemi[biodurum][0],BiyologSistemi[biodurum][4],BiyologSistemi[biodurum][5], bioruhtasi);
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_COMMAND, "biyolog %d %d %d %d %d %d %d", BiyologSistemi[biodurum][0], bioverilen, BiyologSistemi[biodurum][1], biokalan, BiyologSistemi[biodurum][4], BiyologSistemi[biodurum][5], bioruhtasi);
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "CH: %d Map Index : %d ", g_bChannel, ch->GetMapIndex());
	}
	
	ch->ChatPacket(CHAT_TYPE_COMMAND, "indexver %d", g_bChannel);
#ifdef ENABLE_SWITCHBOT
