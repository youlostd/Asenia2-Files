...
LPITEM ITEM_MANAGER::CreateItem(DWORD vnum, short count, DWORD id, bool bTryMagic, int iRarePct, bool bSkipSave)
{
	if (0 == vnum)
		return NULL;
	...
	if (id == 0) // »õ·Î ¸¸µå´Â ¾ÆÀÌÅÛÀÏ ¶§¸¸ Ã³¸®
	{
		...
		else if (BUFF_BK == vnum || BUFF_SOULSTONE == vnum)
		{
			DWORD buffskill;
			do
			{
				buffskill = number(245, 249);
				if (NULL != CSkillManager::instance().Get(buffskill))
					break;
			} while (true);

			item->SetSocket(0, buffskill);
		}
	}
...
void ITEM_MANAGER::CreateQuestDropItem(LPCHARACTER pkChr, LPCHARACTER pkKiller, std::vector<LPITEM> & vec_item, int iDeltaPercent, int iRandRange)
{
	...
	if (quest::CQuestManager::instance().GetEventFlag("drop_futbol"))
	{
		const DWORD ITEM_VNUM = 50096;
