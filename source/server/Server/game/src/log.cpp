...
void LogManager::TalkingLog(BYTE bChannel, BYTE bMapIndex, const char * pszText)
{
//	m_sql.EscapeString(__escape_hint, sizeof(__escape_hint), pszText, strlen(pszText));

//	Query("INSERT INTO talking_log%s VALUES(NOW(), %d, %d,'%s')", get_table_postfix(), bChannel, bMapIndex, __escape_hint);
return;
}
