...
void CSwitchbot::SwitchItems()
{
	for (BYTE bSlot = 0; bSlot < SWITCHBOT_SLOT_COUNT; ++bSlot)
	{
		...
		if (CheckItem(pkItem, bSlot))
		{
			...
			{
				char buf[255];
				int len = snprintf(buf, sizeof(buf), LC_TEXT("%s Ýtemi %d Slotta Baþarýyla Gerçekleþtirildi."), pkItem->GetName(), bSlot + 1);
		...
		else
		{
			...
			if (SWITCHBOT_PRICE_TYPE == 1)
			{
				for (const auto& itemVnum : c_arSwitchingItems)
				{
					...
					if (itemVnum == 71084)
					{
						if ((pkItem->GetType() == ITEM_WEAPON) || (pkItem->GetType() == ARMOR_HEAD) || (pkItem->GetType() == ARMOR_SHIELD) || (pkItem->GetType() == ARMOR_WRIST) || (pkItem->GetType() == ARMOR_FOOTS) || (pkItem->GetType() == ARMOR_EAR) || (pkItem->GetType() == ARMOR_NECK) || (pkItem->GetType() == ITEM_ARMOR && pkItem->GetSubType() == ARMOR_BODY))
						{
							bool bCanUse = true;
							for (int i = 0; i < ITEM_LIMIT_MAX_NUM; ++i)
							{
								if (pkItem->GetLimitType(i) == LIMIT_LEVEL && pkItem->GetLimitValue(i) > 120)
								{
									bCanUse = false;
									break;
								}
			...
			if (stop)
			{
				...
				if (SWITCHBOT_PRICE_TYPE == 1)
				{
					pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Efsunlama islemi icin Efsun nesnesi yok."));
				}
				else
				{
