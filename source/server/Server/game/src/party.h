...
class CParty
{
	...
	public:
		...
		void		SendMessage(LPCHARACTER ch, BYTE bMsg, int64_t dwArg1, DWORD dwArg2);
...
struct FPartyDropDiceRoll
{
	...
	FPartyDropDiceRoll(const LPITEM itemDrop, LPCHARACTER itemOwner) :
		m_itemDrop(itemDrop), m_itemOwner(itemOwner), m_lastNumber(0) {};
	...
	void Process(const LPCHARACTER mobVictim)
	{
		if ((!mobVictim || (mobVictim->GetMobRank() >= MOB_RANK_BOSS && mobVictim->GetMobRank() <= MOB_RANK_KING)) && m_itemOwner->GetParty() && m_itemOwner->GetParty()->GetNearMemberCount() > 1)
		{
			...
			pParty->ChatPacketToAllMember(CHAT_TYPE_DICE_INFO, LC_TEXT("*** Rolling for the following item: %s ***"), m_itemDrop->GetName());

			pParty->ForEachNearMember(*this);
			if (m_itemOwner)
			{
				m_itemDrop->SetOwnership(m_itemOwner);
				pParty->ChatPacketToAllMember(CHAT_TYPE_DICE_INFO, LC_TEXT("*** Rolling of %s: %s ***"), m_itemDrop->GetName(), m_itemOwner->GetName());
			}
		}
		else
			m_itemDrop->SetOwnership(m_itemOwner);
	...
	LPCHARACTER GetItemOwner() {return m_itemOwner;}
	const LPITEM GetItemDrop() {return m_itemDrop;}
	...
	void operator () (LPCHARACTER ch)
	{
		...
		while (true)
		{
			...
			pParty->ChatPacketToAllMember(CHAT_TYPE_DICE_INFO, LC_TEXT("*** -> %20s - Dice score: %05d ***"), ch->GetName(), pickedNumber);
			break;
		}
	}
};
