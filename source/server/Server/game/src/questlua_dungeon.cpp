...
namespace quest
{
	...
	int dungeon_unique_get_hp(lua_State* L)
	{
		if (!lua_isstring(L,1))
		{
			lua_pushnumber(L,0);
	...
	int dungeon_unique_get_max_hp(lua_State* L)
	{
		if (!lua_isstring(L,1))
		{
			lua_pushnumber(L,0);
	...
	struct FPurgeArea
	{
		...
		void operator () (LPENTITY ent)
		{
			if (true == ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (!pChar->IsPet()&& !pChar->IsSupport() && (true == pChar->IsMonster() || true == pChar->IsStone())
	...
	int dungeon_count_stone_byvnum(lua_State* L)
	{
		if (!lua_isnumber(L,1))
			return 0;
		...
		if (pDungeon)
		{
			lua_pushnumber(L, pDungeon->CountStoneByVnum((int)lua_tonumber(L,1)));
			return 1;
		}
	...
	void RegisterDungeonFunctionTable() 
	{
		luaL_reg dungeon_functions[] = 
		{
			...
			{ "unique_get_max_hp",		dungeon_unique_get_max_hp},
...
#endif
			{ "count_stone_byvnum",		dungeon_count_stone_byvnum		},
