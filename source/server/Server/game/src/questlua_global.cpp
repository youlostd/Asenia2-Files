...
namespace quest
{
	...
	struct FKillSectree2
	{
		void operator () (LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (!ch->IsPC() && !ch->IsPet()&& !ch->IsSupport() && !ch->IsNewPet())
#else
				if (!ch->IsPC() && !ch->IsPet())
#endif
					ch->Dead();
	...
	void RegisterGlobalFunctionTable(lua_State* L)
	{
		...
		luaL_reg global_functions[] =
		{
			...
			{	"get_table_postfix",			_get_table_postfix2				},
			{	"mysql_direct_query",			_mysql_direct_query2			},
			{	"mysql_escape_string",			_mysql_escape_string2			},
