...
namespace quest
{
	...
	int npc_set_meley_affect(lua_State* L)
	{
		CQuestManager& q = CQuestManager::instance();
		...
		if (targetChar)
		{
			if (attack_mul == 0)
			{
				targetChar->RemoveAffect(AFFECT_STATUE);
			}
			else if (attack_mul == 1)
			{
				targetChar->AddAffect(AFFECT_STATUE, POINT_NONE, 0, AFF_STATUE1, 3600, 0, true);
			}
			else if (attack_mul == 2)
			{
				targetChar->AddAffect(AFFECT_STATUE, POINT_NONE, 0, AFF_STATUE2, 3600, 0, true);
			}
			else if (attack_mul == 3)
			{
				targetChar->AddAffect(AFFECT_STATUE, POINT_NONE, 0, AFF_STATUE3, 3600, 0, true);
			}
			else if (attack_mul == 4)
			{
				targetChar->AddAffect(AFFECT_STATUE, POINT_NONE, 0, AFF_STATUE4, 3600, 0, true);
			}
			else if (attack_mul == 5)
			{
				targetChar->AddAffect(AFFECT_STATUE, POINT_NONE, 0, AFF_STATUE5, 3600, 0, true);
			}
		}
		return 0;
	}	
	...
	void RegisterNPCFunctionTable()
	{
		luaL_reg npc_functions[] = 
		{
			...
			{ "set_meley_affect",		npc_set_meley_affect	},
			// X-mas santa special
