...
namespace quest 
{
	...
	int pc_get_petvnum(lua_State * L)  
	{
		lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetPlayerPetVnum());
		return 1;
	}

	int pc_set_petvnum(lua_State * L)  
	{
		if (!lua_isnumber(L, 1))
		{
			sys_err("invalid argument");
		...
		else
		{
			...
			ch->SetPlayerPetVnum(newIndex);

			return 0;
		}
	}
	
	int pc_get_petslot(lua_State * L)  
	{
		lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetPlayerPetSlot());
		return 1;
	}

	int pc_set_petslot(lua_State * L)  
	{
		if (!lua_isnumber(L, 1))
		{
			sys_err("invalid argument");
...
#ifdef YANG_LIMIT
		ch->GoldChange(static_cast<LDWORD>(iAmount),"GIVE_GOLD");
#else
		...
		for (int i = 0; i < count; i++)
		{
			if (!item_gets[i])
			{
				if (dwVnums[i] == 1)
				{
					...
					ch->ChatPacket(CHAT_TYPE_MONEY_INFO, LC_TEXT("µ· %d ³ÉÀ» È¹µæÇß½À´Ï´Ù."), dwCounts[i]);
#else
				...
				{
#if defined(__CHATTING_WINDOW_RENEWAL__)
					ch->ChatPacket(CHAT_TYPE_EXP_INFO, LC_TEXT("³ª¹«¿¡¼­ ºÎÅÍ ½ÅºñÇÑ ºûÀÌ ³ª¿É´Ï´Ù."));
					ch->ChatPacket(CHAT_TYPE_EXP_INFO, LC_TEXT("%dÀÇ °æÇèÄ¡¸¦ È¹µæÇß½À´Ï´Ù."), dwCounts[i]);
#else
...
#ifdef YANG_LIMIT
		if (static_cast<ULDWORD>(gold) + ch->GetGold() < 0) { sys_err("QUEST wrong ChangeGold %d (now %lld)", gold, ch->GetGold()); }
		else{
			DBManager::instance().SendMoneyLog(MONEY_LOG_QUEST, ch->GetPlayerID(), gold);
			ch->GoldChange(static_cast<LDWORD>(gold), "pc_change_money");
		}
#else
...
teleport_area:
	...
	int pc_give_award_socket(lua_State* L)
	{
		...
		DBManager::instance().Query("INSERT INTO item_award (login, vnum, count, given_time, why, mall, socket0, socket1, socket2, socket3, socket4, socket5)select '%s', %d, %d, now(), '%s', 1, %s, %s, %s from DUAL where not exists (select login, why from item_award where login = '%s' and why  = '%s') ;", 
			ch->GetDesc()->GetAccountTable().login,
			dwVnum, 
			icount, 
			lua_tostring(L,3),
	...
	int pc_is_busy(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		bool durum = false;
		if (ch->GetExchange() || ch->GetMyShop() || ch->GetShopOwner() || ch->IsOpenSafebox() || ch->IsCubeOpen() || ch->GetOfflineShopOwner() || ch->IsSashCombinationOpen() || ch->IsSashAbsorptionOpen())
		{
			durum = true;
		
		}
		lua_pushboolean(L, durum);
		return 1;
	}	
	...
	int pc_set_rebirt_level(lua_State * L)  
	{
		if (!lua_isnumber(L, 1))
		{
			sys_err("invalid argument");
		...
		{
			int newrebirt = lua_tonumber(L, 1);
			LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();


			sys_log(0,"QUEST [rebirt] %s jumpint to rebþrt %d", ch->GetName(), (int)rint(lua_tonumber(L,1)));


			ch->PointChange(POINT_REBIRT, newrebirt, true);
	...
	#endif
	int pc_get_rebirt(lua_State * L)
	{ 
		lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetRebirt());
		return 1;
	}
#ifdef ENABLE_AURA_SYSTEM
	int pc_open_aura(lua_State * L)
	{
		if (lua_isboolean(L, 1))
		{
			CQuestManager& qMgr = CQuestManager::instance();
	...
	int pc_give_red_gaya(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		...
		DBManager::instance().SendMoneyLog(MONEY_LOG_QUEST, ch->GetPlayerID(), iAmount);
		ch->PointChange(POINT_RED_GAYA, iAmount, true);
		return 0;
	}
	int pc_get_red_gaya(lua_State * L)
	{ 
		lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetRedGaya());
		return 1;
	}
	int pc_change_red_gaya(lua_State * L)
	{
		int red_gaya = (int)lua_tonumber(L, -1);

		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();

		if (red_gaya + ch->GetRedGaya() < 0)
			sys_err("QUEST wrong ChangeRedGaya %d (now %d)", red_gaya, ch->GetRedGaya());
		else
		{
			DBManager::instance().SendMoneyLog(MONEY_LOG_QUEST, ch->GetPlayerID(), red_gaya);
			ch->PointChange(POINT_RED_GAYA, red_gaya, true);
		}

		return 0;
	}
#ifdef OTOMATIK_AV
	int pc_oto_av(lua_State* L) {
		...
		std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery("UPDATE account.account SET premium_oto_av = DATE_ADD((IF(CURRENT_TIMESTAMP() >= premium_oto_av, CURRENT_TIMESTAMP(), premium_oto_av)), INTERVAL 12 HOUR) WHERE id = %d", ch->GetAID()));

		if (msg->uiSQLErrno != 0) { lua_pushnumber(L, 0); }
	...
	int pc_is_blocked(lua_State * L)
	{
		...
		lua_pushboolean(L, MessengerManager::instance().CheckMessengerList(ch->GetName(), tch->GetName(), MESSENGER_BLOCK));
		return 1;
	}
	...
	int pc_is_friend(lua_State * L)
	{
		...
		lua_pushboolean(L, MessengerManager::instance().CheckMessengerList(ch->GetName(), tch->GetName(), MESSENGER_FRIEND));
		return 1;
	}
#endif
	...
	void RegisterPCFunctionTable()
	{
		luaL_reg pc_functions[] = 
		{
			...
			{ "get_petslot",		pc_get_petslot		},
			{ "set_petslot",		pc_set_petslot		},
			
			{ "get_petvnum",		pc_get_petvnum		},
			{ "set_petvnum",		pc_set_petvnum		},			
			...
			{ "get_red_gaya",		pc_get_red_gaya		},
			{ "change_red_gaya",		pc_change_red_gaya		},
			{ "give_red_gaya",		pc_give_red_gaya		},
#ifdef OTOMATIK_AV
			{ "oto_av",pc_oto_av },
#endif
#if defined(ENABLE_MESSENGER_BLOCK)
