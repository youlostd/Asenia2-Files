...
EVENTFUNC(dungeon_regen_event)
{
	...
	if (info->stone_kill != -1 && info->stone_kill != 0)
	{
		if (pDungeon->CountStoneByVnum(8019) != 0)
			return PASSES_PER_SEC(regen->time);

		info->stone_kill--;
		if (info->stone_kill <= 0)
		{
			pDungeon->ClearRegen();
			pDungeon->SetFlag("level", 8);
			pDungeon->SetFlag("save_level", 8);

			pDungeon->JumpAll(pDungeon->GetMapIndex(),  2048+590, 6656+403);
			pDungeon->Purge();
			pDungeon->CheckEliminated();
			pDungeon->SpawnRegen("data/dungeon/deviltower8_regen.txt", false);

			pDungeon->SpawnMob(20366, 640, 460);
			pDungeon->Notice("Kulede metin kalmadi.");
			return 0;
		}
		else
		{
			std::string dgNotice = "Kalan Katil Metini: " + std::to_string(info->stone_kill);
			pDungeon->Notice(dgNotice.c_str());
		}
	}
	...
	while (true)
	{
			...
			tmp.type == REGEN_TYPE_ANYWHERE)
		{
			...
			if (!bOnce && pDungeon != NULL)
			{
				...
				info->stone_kill = ((lMapIndex == 66 || (lMapIndex / 10000) == 66) && regen->vnum == 8019) ? 200 : -1;
