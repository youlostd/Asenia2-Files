...
// #endif
...
#ifdef YANG_LIMIT
	ULDWORD dwPrice = static_cast<ULDWORD>(r_item.price);
#else
	DWORD dwPrice = r_item.price;
#endif
#ifdef PRICE_TYPE
	...
	if (it->second)	// if other empire, price is triple
		...
		if (dwPriceType == 0)
		{
			if (ch->GetGold() < static_cast<ULDWORD>(dwPrice) && ch->GetCheque() >= (int) dwPriceCheque)
			{
				sys_log(1, "Shop::Buy : Not enough money : %s has %d, price %d", ch->GetName(), ch->GetGold(), dwPrice);
				return SHOP_SUBHEADER_GC_NOT_ENOUGH_MONEY;
			}

			if (ch->GetCheque() < (int) dwPriceCheque && ch->GetGold() >= static_cast<ULDWORD>(dwPrice))
			{
				sys_log(1, "Shop::Buy : Not enough won : %s has %d, price_cheque %d", ch->GetName(), ch->GetCheque(), dwPriceCheque);
				return SHOP_SUBHEADER_GC_NOT_ENOUGH_CHEQUE;
			}
...
#else
	if (ch->GetGold() < static_cast<ULDWORD>(dwPrice))
	{
		sys_log(1, "Shop::Buy : Not enough money : %s has %d, price %d", ch->GetName(), ch->GetGold(), dwPrice);
		return SHOP_SUBHEADER_GC_NOT_ENOUGH_MONEY;
	}
	...
	else if (dwPriceType == 3)
	{
		...
		//LogManager::instance().WonLog(ch->GetPlayerID(), ch->GetName(), item->GetID(), item->GetName(), dwPrice);
	}
	else
	{
	...
	if (m_pkPC)
	{
		...
		if (dwPriceType == 0)
		{
			...
			m_pkPC->GoldChange(static_cast<LDWORD>(-dwPrice));
#ifdef ENABLE_CHEQUE_SYSTEM
		...
		else
		{
			...
			m_pkPC->ChatPacket(1, "%ld" , dwPrice);
		}
#else
		// m_pkPC->PointChange(POINT_GOLD, -dwPrice, false);
		m_pkPC->GoldChange(static_cast<LDWORD>(-dwPrice));
...
bool CShop::AddGuest(LPCHARACTER ch, DWORD owner_vid, bool bOtherEmpire, bool uzak_market)
#else
