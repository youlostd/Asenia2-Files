...
#endif
{
	...
	if (IS_SET(item->GetAntiFlag(), ITEM_ANTIFLAG_SELL_WITH_METIN))
		return;
...
#ifdef YANG_LIMIT
	const ULDWORD nTotalMoney = static_cast<ULDWORD>(ch->GetGold()) + static_cast<ULDWORD>(dwPrice);
	if (ch->GetAllowedGold() <= nTotalMoney){
		sys_err("[OVERFLOW_GOLD] id %u name %s gold ll%u", ch->GetPlayerID(), ch->GetName(), ch->GetGold());
...
#ifdef YANG_LIMIT
	ch->GoldChange(static_cast<LDWORD>(dwPrice));
#else
...
bool CShopManager::NPCAC(LPCHARACTER pkChr, LPCHARACTER pkChrShopKeeper, int iShopVnum)
{
	...
	if (pkChr->IsObserverMode())
		return false;

	if (pkChr->GetExchange() || pkChr->GetShopOwner() || pkChr->GetMyShop() || pkChr->IsOpenSafebox() || pkChr->IsCubeOpen() || pkChr->GetOfflineShopOwner()
#ifdef WJ_SECURITY_SYSTEM
		|| pkChr->IsActivateSecurity() == true
#endif
#ifdef __SASH_SYSTEM__
		|| pkChr->IsSashCombinationOpen() || pkChr->IsSashAbsorptionOpen()
#endif
	...
	{
		pkChr->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You should close the open windows"));
		return false;
	}
	...
	pkShop->AddGuest(pkChr, pkChrShopKeeper->GetVID(), true);
	pkChr->SetShopOwner(pkChrShopKeeper);
