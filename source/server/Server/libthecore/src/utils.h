...
#define TOKEN(string) if (!str_cmp(token_string, string))
    ...
    extern int64_t MAX(int64_t a, int64_t b); // 둘중에 큰 값을 리턴
    extern int64_t MIN(int64_t a, int64_t b); // 둘중에 작은 값을 리턴
    extern int64_t MINMAX(int64_t min, int64_t value, int64_t max); // 최소 최대 값을 함께 비교해서 리턴
